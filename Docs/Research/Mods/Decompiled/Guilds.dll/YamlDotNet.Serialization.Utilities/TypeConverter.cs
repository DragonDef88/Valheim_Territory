...
internal static class TypeConverter
{
	...
	{
		if (value == null || ReflectionExtensions.IsDbNull(value))
		{
			if (!ReflectionExtensions.IsValueType(destinationType))
			{
				return null;
			}
		...
		if (ReflectionExtensions.IsGenericType(destinationType) && destinationType.GetGenericTypeDefinition() == typeof(Nullable<>))
		{
		...
		if (ReflectionExtensions.IsEnum(destinationType))
		{
		...
		{
			foreach (MethodInfo publicStaticMethod2 in ReflectionExtensions.GetPublicStaticMethods(array[i]))
			{
		...
		if (type == typeof(string))
		{
			try
			{
				MethodInfo publicStaticMethod = ReflectionExtensions.GetPublicStaticMethod(destinationType, "Parse", typeof(string), typeof(IFormatProvider));
