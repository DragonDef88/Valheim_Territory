...
internal sealed class ReadablePropertiesTypeInspector : TypeInspectorSkeleton
{
	private sealed class ReflectionPropertyDescriptor : IPropertyDescriptor
	{
		...
		{
			return (T)ReflectionExtensions.GetAllCustomAttributes<T>(propertyInfo).FirstOrDefault();
		}

		public IObjectDescriptor Read(object target)
		{
			object obj = PropertyInfoExtensions.ReadValue(propertyInfo, target);
	...
	{
		return ReflectionExtensions.GetProperties(type, includeNonPublicProperties).Where(IsValidProperty).Select((Func<PropertyInfo, IPropertyDescriptor>)((PropertyInfo p) => new ReflectionPropertyDescriptor(p, typeResolver)));
	}
}
