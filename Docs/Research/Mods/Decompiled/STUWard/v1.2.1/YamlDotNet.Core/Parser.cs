...
internal class Parser : IParser
{
	...
	private ParsingEvent ParseNode(bool isBlock, bool isIndentlessSequence)
	{
			...
			else
			{
				if (!tag.IsEmpty || !(token is Tag tag3))
				{
					if (token is Anchor { Start: var start3 } anchor4)
					{
						...
						throw new SemanticErrorException(in start3, in end, "While parsing a node, found more than one anchor.");
					}
					if (token is YamlDotNet.Core.Tokens.AnchorAlias { Start: var start4 } anchorAlias2)
					{
						end = anchorAlias2.End;
						throw new SemanticErrorException(in start4, in end, "While parsing a node, did not find expected token.");
					}
					...
					throw new SemanticErrorException(in start5, in end, error2.Value);
				}
				tag2 = tag3;
				...
				else
				{
					if (!tagDirectives.Contains(tag3.Handle))
					{
						...
						throw new SemanticErrorException(in start5, in end, "While parsing a node, found undefined tag handle.");
					}
		...
		if (token is YamlDotNet.Core.Tokens.Scalar scalar)
		{
			...
			if (!anchor.IsEmpty && scanner.MoveNextWithoutConsuming())
			{
				...
				if (currentToken is Error)
				{
					...
					throw new SemanticErrorException(in start5, in end, error3.Value);
				}
			}
			if (state == ParserState.FlowMappingKey && !(scanner.Current is FlowMappingEnd) && scanner.MoveNextWithoutConsuming())
			{
				...
				if (currentToken != null && !(currentToken is FlowEntry) && !(currentToken is FlowMappingEnd))
				{
					...
					throw new SemanticErrorException(in start5, in end, "While parsing a flow mapping, did not find expected ',' or '}'.");
				}
			}
			return result2;
		}
		...
		throw new SemanticErrorException(in start5, in end, "While parsing a node, did not find expected node content.");
	}
	...
	private ParsingEvent ParseBlockSequenceEntry(bool isFirst)
	{
		...
		if (token is BlockEnd blockEnd)
		{
			...
			ParsingEvent result = new SequenceEnd(in start, in end);
			Skip();
			return result;
		}
		start = token?.Start ?? Mark.Empty;
	...
	private ParsingEvent ParseBlockMappingKey(bool isFirst)
	{
		...
		if (token is BlockEnd blockEnd)
		{
			...
			ParsingEvent result = new MappingEnd(in position2, in end);
			Skip();
			return result;
		}
		if (GetCurrentToken() is Error { Start: var start } error)
		{
			...
			throw new SyntaxErrorException(in start, in end, error.Value);
		}
	...
	private ParsingEvent ParseBlockMappingValue()
	{
		...
		state = ParserState.BlockMappingKey;
		Mark position2 = token?.Start ?? Mark.Empty;
		return ProcessEmptyScalar(in position2);
	}
	...
	private ParsingEvent ParseFlowSequenceEntry(bool isFirst)
	{
		...
		result = new SequenceEnd(in start, in end);
		Skip();
		return result;
	}
