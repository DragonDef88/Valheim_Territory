...
internal class Scanner : IScanner
{
	...
	private void FetchDocumentIndicator(bool isStartToken)
	{
		...
		if (isStartToken)
		{
			...
			Mark end = cursor.Mark();
			insertionQueue.Enqueue(new DocumentStart(in start, in end));
			return;
		}
		Token token = null;
		while (!analyzer.EndOfInput && !analyzer.IsBreak() && !analyzer.Check('#'))
		{
			if (!analyzer.IsWhite())
			{
				token = new Error("While scanning a document end, found invalid content after '...' marker.", start, cursor.Mark());
				break;
			}
			Skip();
		}
		tokens.Enqueue(new DocumentEnd(in start, in start));
		if (token != null)
		{
			tokens.Enqueue(token);
		}
	...
	private void FetchQuotedScalar(bool isSingleQuoted)
	{
		...
		Scalar item = ScanFlowScalar(isSingleQuoted);
		tokens.Enqueue(item);
		lastScalar = item;
