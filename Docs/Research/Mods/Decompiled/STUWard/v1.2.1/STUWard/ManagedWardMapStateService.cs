...
internal static class ManagedWardMapStateService
{
	...
	private sealed class ManagedWardMapEvent
	{
		...
		internal long[] PermittedPlayerIds { get; set; } = Array.Empty<long>();
	...
	internal static void NotifyLiveWardMutation(PrivateArea? area, ManagedWardMapMutationKind kind, string reason, bool liveDisplayRefresh = false)
	{
	...
	internal static void NotifyZdoWardMutation(ZDO? zdo, ManagedWardMapMutationKind kind, string reason, bool liveDisplayRefresh = false)
	{
	...
	internal static void NotifySyncedWardState(ZDOID zdoId, uint dataRevision, long ownerPlayerId, int wardGuildId, Vector3 position, float radius, bool isEnabled, long[] permittedPlayerIds, string reason, bool liveDisplayRefresh = false)
	{
	...
	internal static void NotifyViewerProjectionChanged(string reason, bool fullRefresh, HashSet<long>? recipientPeerUids = null, bool refreshImmediatelyIfVisible = false)
	{
	...
	{
		bool visibilityIndexChanged = ApplyVisibilityIndexEvent(mapEvent);
	...
	private static bool ApplyVisibilityIndexEvent(ManagedWardMapEvent mapEvent)
	{
		...
		{
			ZDOID zdoId = mapEvent.ZdoId;
			if (!((ZDOID)(ref zdoId)).IsNone())
			{
				return WardMinimapVisibilityIndex.ForgetWard(mapEvent.ZdoId);
			}
			return false;
		}
		case ManagedWardMapEventKind.SyncedWardState:
			WardMinimapVisibilityIndex.ObserveSyncedWardState(mapEvent.ZdoId, mapEvent.DataRevision, mapEvent.OwnerPlayerId, mapEvent.WardGuildId, mapEvent.Position, mapEvent.Radius, mapEvent.IsEnabled, mapEvent.PermittedPlayerIds);
			return true;
	...
	private static void ApplyPinsEvent(ManagedWardMapEvent mapEvent, bool visibilityIndexChanged)
	{
		...
		case ManagedWardMapEventKind.ViewerProjectionChanged:
			...
			{
				WardMinimapPinsManager.QueueServerViewerRefreshRecipients(mapEvent.FullRefresh ? null : mapEvent.RecipientPeerUids, mapEvent.Reason);
			}
			break;
	...
	private static bool ShouldUpdateIndex(ManagedWardMapMutationKind kind)
	{
		if (kind != 0)
		{
			return kind == ManagedWardMapMutationKind.IndexAndPins;
		}
		return true;
	}
	...
	{
		if (kind != ManagedWardMapMutationKind.PinsOnly)
		{
			return kind == ManagedWardMapMutationKind.IndexAndPins;
		}
		return true;
