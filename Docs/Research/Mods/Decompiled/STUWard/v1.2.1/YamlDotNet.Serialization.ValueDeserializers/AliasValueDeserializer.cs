...
internal sealed class AliasValueDeserializer : IValueDeserializer
{
	...
	public object? DeserializeValue(IParser parser, Type expectedType, SerializerState state, IValueDeserializer nestedObjectDeserializer)
	{
		...
		if (parser.Accept<NodeEvent>(out var event2) && !event2.Anchor.IsEmpty)
		{
			...
			if (!aliasState2.ContainsKey(anchorName))
			{
				aliasState2[anchorName] = new ValuePromise(new AnchorAlias(anchorName));
			}
		}
		...
		if (!anchorName.IsEmpty)
		{
			...
			if (!aliasState3.TryGetValue(anchorName, out ValuePromise value2))
			{
				aliasState3.Add(anchorName, new ValuePromise(obj));
			}
			...
			{
				aliasState3[anchorName] = new ValuePromise(obj);
			}
		}
		return obj;
	}
