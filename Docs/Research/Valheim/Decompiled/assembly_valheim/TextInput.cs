...
public class TextInput : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (m_visibleFrame && !Console.IsVisible() && !Chat.instance.HasFocus() && ZInput.GetKeyDown((KeyCode)27, true))
		{
			Hide();
		}
	}
	...
	public void OnInput()
	{
		setText(((TMP_InputField)m_inputField).text.Replace("\\n", "\n").Replace("\\t", "\t"));
	...
	private void Show(string topic, string text, int charLimit)
	{
		...
		((TMP_InputField)m_inputField).characterLimit = charLimit;
		((TMP_InputField)m_inputField).text = text;
