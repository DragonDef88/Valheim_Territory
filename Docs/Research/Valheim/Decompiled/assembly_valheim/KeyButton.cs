...
public class KeyButton : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler
{
	...
	public void Awake()
	{
		...
		((UnityEvent)m_button.onClick).AddListener(new UnityAction(OnClick));
	}

	private void Update()
	{
		if (ZInput.IsGamepadActive() && (Object)(object)EventSystem.current.currentSelectedGameObject == (Object)(object)((Component)m_button).gameObject)
		{
			UpdateTooltip();
		}
	}
	...
	public bool TryMatch(IReadOnlyList<string> keys)
	{
		...
		{
			if (!StringUtils.ContainsString(keys, m_keys[i], StringComparison.InvariantCultureIgnoreCase))
			{
				return false;
			}
		}
	...
	{
		return ((Component)this).gameObject.GetComponentInChildren<TMP_Text>().text;
	}
