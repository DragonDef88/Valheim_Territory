...
public class CreatureSpawner : MonoBehaviour
{
	public class Group : HashSet<CreatureSpawner>
	{
		...
		public void CountSpawns(out int spawnedNow, out int spawnedEver)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if ((Object)(object)current.m_nview == (Object)null || current.m_nview.GetZDO() == null)
				{
					continue;
				}
		...
		public void SpawnWeighted()
		{
			...
			ZLog.LogError((object)"No active spawners for group but something is still calling it!");
		}
	}
	...
	private void Awake()
	{
		...
		{
			((MonoBehaviour)this).InvokeRepeating("UpdateSpawner", (float)Random.Range(m_spawnInterval / 2, m_spawnInterval), (float)m_spawnInterval);
		}
	}
	...
	private void UpdateSpawner()
	{
		...
		{
			m_location = Location.GetLocation(((Component)this).transform.position);
			m_checkedLocation = true;
			if (Object.op_Implicit((Object)(object)m_location) && m_location.m_blockSpawnGroups.Contains(m_spawnGroupID))
			{
				m_nview.Destroy();
				return;
			}
		...
		if (!m_spawnInPlayerBase && Object.op_Implicit((Object)(object)EffectArea.IsPointInsideArea(((Component)this).transform.position, EffectArea.Type.PlayerBase)))
		{
			return;
		}
		if (m_triggerNoise > 0f)
		{
			if (!Player.IsPlayerInRange(((Component)this).transform.position, m_triggerDistance, m_triggerNoise))
			{
				return;
			}
		}
		else if (!Player.IsPlayerInRange(((Component)this).transform.position, m_triggerDistance))
		{
			return;
		}
	...
	private ZNetView Spawn()
	{
		...
		GameObject val2 = Object.Instantiate<GameObject>(m_creaturePrefab, position, val);
		ZNetView component = val2.GetComponent<ZNetView>();
		if (m_wakeUpAnimation)
		{
			val2.GetComponent<ZSyncAnimation>()?.SetBool("wakeup", value: true);
		}
		BaseAI component2 = val2.GetComponent<BaseAI>();
		if ((Object)(object)component2 != (Object)null && m_setPatrolSpawnPoint)
		{
		...
		if ((Object)(object)m_location != (Object)null && !m_location.m_excludeEnemyLevelOverrideGroups.Contains(m_spawnGroupID))
		{
		...
		if (num2 > 1)
		{
			...
			if (Object.op_Implicit((Object)(object)component3))
			{
				...
				for (i = num; i < num2; i++)
				{
					if (!(Random.Range(0f, 100f) <= SpawnSystem.GetLevelUpChance(num3)))
					{
						break;
					}
				}
			...
			{
				ItemDrop component4 = val2.GetComponent<ItemDrop>();
	...
	private void SpawnEffect(GameObject spawnedObject)
	{
		...
		Vector3 basePos = (Object.op_Implicit((Object)(object)component) ? component.GetCenterPoint() : (((Component)this).transform.position + Vector3.up * 0.75f));
		...
		if (m_spawnGroup == null)
		{
			...
			while (m_groupNew.Count > 0)
			{
				...
				for (int num = m_groupUnchecked.Count - 1; num >= 0; num--)
				{
					...
					if (creatureSpawner.m_spawnGroupID == creatureSpawner2.m_spawnGroupID && Vector3.Distance(((Component)creatureSpawner).transform.position, ((Component)creatureSpawner2).transform.position) <= creatureSpawner.m_spawnGroupRadius + creatureSpawner2.m_spawnGroupRadius)
					{
			...
			if (m_groups.Count > 0)
			{
				if (m_groups.Count > 1)
				{
					ZLog.Log((object)string.Format("{0} {1} merged for {2} spawners.", m_groups.Count, "Group", m_grouped.Count));
				}
				group = m_groups[0];
			}
			else
			...
			ZLog.Log((object)string.Format("{0} created for {1} spawners.", "Group", group.Count));
		}
