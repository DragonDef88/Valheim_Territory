...
public class Game : MonoBehaviour
{
	...
	private static FileSource m_profileFileSource = (FileSource)1;
	...
	private State m_spawnRandomState;
	...
	private void Awake()
	{
		...
		{
			PortalPrefabHash.Add(StringExtensionMethods.GetStableHashCode(((Object)portalPrefab).name));
		}
		if (FejdStartup.AwakePlatforms())
		{
			...
			{
				Object.Instantiate<GameObject>(m_serverOptionPrefab);
			}
			Settings.ApplyStartupSettings();
			if (string.IsNullOrEmpty(m_profileFilename))
			{
				m_playerProfile = new PlayerProfile(m_devProfileName, (FileSource)1);
			...
			((MonoBehaviour)this).InvokeRepeating("CollectResourcesCheckPeriodic", 3600f, 3600f);
			Gogan.LogEvent("Screen", "Enter", "InGame", 0L);
			Gogan.LogEvent("Game", "InputMode", ZInput.IsGamepadActive() ? "Gamepad" : "MK", 0L);
			ZLog.Log((object)("isModded: " + isModded));
		}
	}
	...
	private void Start()
	{
		...
		if (ZNet.instance.IsServer())
		{
			...
			((MonoBehaviour)this).InvokeRepeating("UpdateSleeping", 2f, 2f);
			((MonoBehaviour)this).StartCoroutine("ConnectPortalsCoroutine");
		}
	...
	{
		if (displayMessage && Object.op_Implicit((Object)(object)Player.m_localPlayer))
		{
			Player.m_localPlayer.Message(MessageHud.MessageType.TopLeft, "Unloading unused assets");
		}
		ZLog.Log((object)"Unloading unused assets");
	...
	public void CollectResourcesCheckPeriodic()
	{
		...
		{
			ZLog.Log((object)"Skipping unloading unused assets");
		}
	}
	...
	public void SavePlayerProfile(bool setLogoutPoint)
	{
		...
		if ((int)m_playerProfile.m_fileSource == 2)
		{
			ulong num = 1048576uL;
			if (FileHelpers.FileExistsCloud(m_playerProfile.GetPath()))
			{
				num += FileHelpers.GetFileSize(m_playerProfile.GetPath(), (FileSource)2);
			}
			num *= 3;
			if (FileHelpers.OperationExceedsCloudCapacity(num))
			{
				string path = m_playerProfile.GetPath();
				m_playerProfile.m_fileSource = (FileSource)1;
				...
				ZLog.LogWarning((object)"The character save operation may exceed the cloud save quota and it has therefore been moved to local storage!");
			}
		}
	...
	private Player SpawnPlayer(Vector3 spawnPoint, bool spawnValkyrie)
	{
		...
		Player component = Object.Instantiate<GameObject>(m_playerPrefab, spawnPoint, Quaternion.identity).GetComponent<Player>();
	...
	private bool FindSpawnPoint(out Vector3 point, out bool usedLogoutPoint, float dt)
	{
		...
		if (!m_respawnAfterDeath && m_playerProfile.HaveLogoutPoint())
		{
			...
			if (m_respawnWait > m_respawnLoadDuration && ZNetScene.instance.IsAreaReady(logoutPoint))
			{
				...
				{
					Vector3 val = logoutPoint;
					ZLog.Log((object)("Invalid spawn point, no ground " + ((object)(Vector3)(ref val)).ToString()));
					m_respawnWait = 0f;
	...
	public void RequestRespawn(float delay, bool afterDeath = false)
	{
		...
		((MonoBehaviour)this).CancelInvoke("_RequestRespawn");
		((MonoBehaviour)this).Invoke("_RequestRespawn", delay);
	}
	...
	private void _RequestRespawn()
	{
		...
		if (Object.op_Implicit((Object)(object)Player.m_localPlayer))
		{
			ZNetScene.instance.Destroy(((Component)Player.m_localPlayer).gameObject);
	...
	private void FixedUpdate()
	{
		...
		{
			Logout();
			ZLog.LogError((object)"World load failed, exiting without save. Check backups!");
		}
	...
	{
		if (m_saveInterval - m_saveTimer > 30f && m_saveInterval - (m_saveTimer + dt) <= 30f && Object.op_Implicit((Object)(object)MessageHud.instance) && ZNet.instance.IsServer())
		{
	...
	private void UpdateRespawn(float dt)
	{
		...
		if (m_firstSpawn)
		{
			...
			if (ZNet.m_loadError)
			{
				...
				((Component)Hud.instance.m_betaText.transform.GetChild(0)).GetComponent<TMP_Text>().text = "WORLD SAVE DISABLED! (World load error)";
	...
	public void SkipIntro()
	{
		...
		m_inIntro = false;
		RequestRespawn(0f);
		if (Object.op_Implicit((Object)(object)Valkyrie.m_instance))
		{
	...
	{
		yield return (object)new WaitForSeconds(sleepInSeconds);
	...
	public static void SetProfile(string filename, FileSource fileSource)
	{
		...
		foreach (ZDO item2 in portals)
		{
			if (!IsCurrentlyConnectingPortal(item2) && item2.GetConnectionZDOID(ZDOExtraData.ConnectionType.Portal).IsNone())
			{
				...
				if (zDO2 != null)
				{
					...
					num++;
					ZLog.Log((object)("Connected portals " + item2?.ToString() + " <-> " + zDO2));
				}
			}
		}
		if (num > 0)
		{
			ZLog.Log((object)("[ Connected " + num + " portals ]"));
		}
	}
	...
	private void SleepStop(long sender)
	{
		...
		{
			ZLog.Log((object)"Saved recently, skipping sleep save.");
		}
	}
	...
	private void RPC_DiscoverClosestLocation(long sender, string name, Vector3 point, string pinName, int pinType, bool showMap, bool discoverAll)
	{
		...
		{
			ZLog.Log((object)$"Found {m_tempLocations.Count} locations of type {name}");
			{
	...
	private void RPC_DiscoverLocationResponse(long sender, string pinName, int pinType, Vector3 pos, bool showMap)
	{
		...
		if (Object.op_Implicit((Object)(object)Player.m_localPlayer) && Minimap.instance.m_mode == Minimap.MapMode.None)
		{
			Player.m_localPlayer.SetLookDir(pos - ((Component)Player.m_localPlayer).transform.position, 3.5f);
		}
	}
	...
	private static void UpdatePause()
	{
		...
		if (IsPaused() && Menu.IsVisible() && Object.op_Implicit((Object)(object)Player.m_localPlayer))
		{
	...
	{
		if ((ZNet.instance.IsServer() && ZNet.instance.GetPeerConnections() > 0) || !Object.op_Implicit((Object)(object)Player.m_localPlayer) || !Object.op_Implicit((Object)(object)ZNet.instance))
		{
			return false;
		}
		if (Player.m_debugMode && !ZNet.instance.IsServer() && Object.op_Implicit((Object)(object)Console.instance) && Console.instance.IsCheatsEnabled())
		{
			return true;
		}
	...
	public int ScaleDrops(ItemDrop.ItemData data, int amount)
	{
		...
		{
			amount = (int)Mathf.Clamp(Mathf.Round((float)amount * m_resourceRate), 1f, (float)((data.m_shared.m_maxStackSize > 1) ? data.m_shared.m_maxStackSize : 1000));
		}
		return amount;
	}
	...
	{
		return (int)Mathf.Max(1f, Mathf.Round(Random.Range((float)randomMin, (float)randomMax) * m_resourceRate));
	}
	...
	{
		List<string> playerKeys = (Object.op_Implicit((Object)(object)Player.m_localPlayer) ? Player.m_localPlayer.GetUniqueKeys() : null);
		...
		for (int i = 0; i < globalKeys.Count; i++)
		{
			...
			{
				Utils.IncrementOrSet<string>(playerProfile.m_knownWorldKeys, string.Format("{0} {1}", globalKeys2, "default"), 1f);
			}
		}
	...
	{
		m_noMap = (Object.op_Implicit((Object)(object)ZoneSystem.instance) && ZoneSystem.instance.GetGlobalKey(GlobalKeys.NoMap)) || ((Object)(object)Player.m_localPlayer != (Object)null && PlatformPrefs.GetFloat("mapenabled_" + Player.m_localPlayer.GetPlayerName(), 1f) == 0f);
		...
		foreach (ItemConversion damageTypeDropConversion in m_damageTypeDropConversions)
		{
			...
			foreach (ItemDrop item in damageTypeDropConversion.m_items)
			{
				if (!(item.m_itemData.m_shared.m_name != itemDrop.m_itemData.m_shared.m_name))
				{
					...
					return ((Component)damageTypeDropConversion.m_result).gameObject;
				}
			}
		}
		return dropPrefab;
