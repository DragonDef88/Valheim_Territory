...
public class WaterVolume : MonoBehaviour
{
	...
	private static readonly Vector2[] s_createWaveDirections;

	private static Vector2[] s_createWaveTangents;

	public static List<WaterVolume> Instances { get; }
	...
	private void Awake()
	{
		...
		{
			s_createWaveTangents = (Vector2[])(object)new Vector2[10]
			{
	...
	{
		if (Object.op_Implicit((Object)(object)m_heightmap))
		{
	...
	{
		((Renderer)m_waterSurface).material.SetFloat(s_shaderWaterTime, s_waterTime);
	}
	...
	private void SetupMaterial()
	{
		if (m_forceDepth >= 0f)
		{
			((Renderer)m_waterSurface).material.SetFloatArray(s_shaderDepth, new float[4] { m_forceDepth, m_forceDepth, m_forceDepth, m_forceDepth });
		}
		else
		{
			((Renderer)m_waterSurface).material.SetFloatArray(s_shaderDepth, m_normalizedDepth);
		}
		((Renderer)m_waterSurface).material.SetFloat(s_shaderUseGlobalWind, m_useGlobalWind ? 1f : 0f);
	}
	...
	public float GetWaterSurface(Vector3 point, float waveFactor = 1f)
	{
		...
		float num3 = ((Component)this).transform.position.y + num + m_surfaceOffset;
	...
	private float CreateWave(Vector3 worldPos, float time, float waveSpeed, float waveLength, float waveHeight, Vector2 dir, Vector2 tangent, float sharpness)
	{
		...
		Vector2 val = -(worldPos.z * dir + worldPos.x * tangent);
		float num = time * waveSpeed;
		return (TrochSin(num + val.y * waveLength, sharpness) * TrochSin(num * 0.123f + val.x * 0.13123f * waveLength, sharpness) - 0.2f) * waveHeight;
	}
	...
	private float CalcWave(Vector3 worldPos, float depth, Vector4 wind, float waterTime, float waveFactor)
	{
		...
		((Vector2)(ref s_createWaveDirections[0])).Normalize();
	...
	public float CalcWave(Vector3 worldPos, float depth, float waterTime, float waveFactor)
	{
		...
		float num = CalcWave(worldPos, depth, s_globalWind1, waterTime, waveFactor);
		float num2 = CalcWave(worldPos, depth, s_globalWind2, waterTime, waveFactor);
		return Mathf.LerpUnclamped(num, num2, s_globalWindAlpha);
	}
	...
	public float Depth(Vector3 point)
	{
		...
		float num = (val.x + size.x / 2f) / size.x;
		float num2 = (val.z + size.z / 2f) / size.z;
		float num3 = Mathf.Lerp(m_normalizedDepth[3], m_normalizedDepth[2], num);
		float num4 = Mathf.Lerp(m_normalizedDepth[0], m_normalizedDepth[1], num);
		return Mathf.Lerp(num3, num4, num2);
	}

	private void OnTriggerEnter(Collider triggerCollider)
	{
		IWaterInteractable component = ((Component)triggerCollider.attachedRigidbody).GetComponent<IWaterInteractable>();
		if (component != null)
		{
	...
	public void UpdateFloaters()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (Object.op_Implicit((Object)(object)transform))
			{
				...
				waterInteractable.SetLiquidLevel(waterSurface, LiquidType.Water, (Component)(object)this);
			}
			else
			{
				s_inWaterRemoveIndices.Add(i);
	...
	private void OnTriggerExit(Collider triggerCollider)
	{
		...
		if (component != null)
		{
			...
			{
				component.SetLiquidLevel(-10000f, LiquidType.Water, (Component)(object)this);
			}
	...
	private void OnDestroy()
	{
		foreach (IWaterInteractable item in m_inWater)
		{
			...
			{
				item.SetLiquidLevel(-10000f, LiquidType.Water, (Component)(object)this);
			}
		}
		m_inWater.Clear();
	}
	...
	private void OnDrawGizmosSelected()
	{
		...
		Gizmos.DrawWireCube(((Component)this).transform.position + Vector3.up * m_surfaceOffset, new Vector3(2f, 0.05f, 2f));
	}
	...
	static WaterVolume()
	{
		...
		Vector2 val = new Vector2(1.0312f, 0.312f);
		array[0] = ((Vector2)(ref val)).normalized;
		val = new Vector2(1.0312f, 0.312f);
		array[1] = ((Vector2)(ref val)).normalized;
		val = new Vector2(-0.123f, 1.12f);
		array[2] = ((Vector2)(ref val)).normalized;
		val = new Vector2(0.423f, 0.124f);
		array[3] = ((Vector2)(ref val)).normalized;
		val = new Vector2(0.123f, -0.64f);
		array[4] = ((Vector2)(ref val)).normalized;
		val = new Vector2(-0.523f, -0.64f);
		array[5] = ((Vector2)(ref val)).normalized;
		val = new Vector2(0.223f, 0.74f);
		array[6] = ((Vector2)(ref val)).normalized;
		val = new Vector2(0.923f, -0.24f);
		array[7] = ((Vector2)(ref val)).normalized;
		val = new Vector2(-0.323f, 0.44f);
		array[8] = ((Vector2)(ref val)).normalized;
		val = new Vector2(0.5312f, -0.812f);
		array[9] = ((Vector2)(ref val)).normalized;
		s_createWaveDirections = (Vector2[])(object)array;
		s_createWaveTangents = null;
		Instances = new List<WaterVolume>();
	}
}
