...
public class ClosedCaptions : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		if ((Object)(object)m_instance == (Object)null)
		{
			...
			m_bgAlpha = ((Graphic)m_image).color.a;
			Color color = ((Graphic)m_image).color;
			color.a = 0f;
	...
	private void Update()
	{
		...
		Color color = ((Graphic)m_image).color;
		float num = ((m_captionItems.Count == 0) ? 0f : m_bgAlpha);
		color.a = Mathf.Lerp(color.a, num, Time.deltaTime / 0.25f);
		((Graphic)m_image).color = color;
	}
