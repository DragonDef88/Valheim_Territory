...
public class TriggerSpawner : MonoBehaviour
{
	...
	public static void TriggerAllInRange(Vector3 p, float range)
	{
		...
		ZLog.Log((object)"Trigging spawners in range");
		foreach (TriggerSpawner allSpawner in m_allSpawners)
		{
			if (Vector3.Distance(((Component)allSpawner).transform.position, p) < range)
			{
				allSpawner.Trigger();
			}
		}
	...
	{
		ZLog.Log((object)("Trigging " + ((Object)((Component)this).gameObject).name));
		TrySpawning();
	}
	...
	private void TrySpawning()
	{
		...
		if (m_minSpawnInterval > 0f)
		{
			...
			if (timeSpan.TotalMinutes < (double)m_minSpawnInterval)
			{
				...
				ZLog.Log((object)("Not enough time passed " + timeSpan2));
				return;
			}
		}
		if (Random.Range(0f, 100f) > m_spawnChance)
		{
			ZLog.Log((object)("Spawn chance fail " + m_spawnChance));
		}
		else
		{
			Spawn();
	...
	private bool Spawn()
	{
		...
		GameObject val = m_creaturePrefabs[Random.Range(0, m_creaturePrefabs.Length)];
		int num = m_maxSpawned + (int)(m_maxExtraPerPlayer * (float)Game.instance.GetPlayerDifficulty(((Component)this).transform.position));
		if (num > 0 && SpawnSystem.GetNrOfInstances(val, ((Component)this).transform.position, m_maxSpawnedRange) >= num)
		{
			return false;
		}
		Quaternion val2 = (m_useSpawnerRotation ? ((Component)this).transform.rotation : Quaternion.Euler(0f, Random.Range(0f, 360f), 0f));
		GameObject val3 = Object.Instantiate<GameObject>(val, position, val2);
		val3.GetComponent<ZNetView>();
		BaseAI component = val3.GetComponent<BaseAI>();
		...
		m_spawnEffects.Create(((Component)this).transform.position, ((Component)this).transform.rotation);
		return true;
	}
