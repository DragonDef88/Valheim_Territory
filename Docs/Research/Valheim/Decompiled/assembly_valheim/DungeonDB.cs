...
public class DungeonDB : MonoBehaviour
{
	...
	public class RoomData
	{
		...
		public Room RoomInPrefab
		{
			get
			{
				...
				if ((Object)(object)m_loadedRoom == (Object)null)
				{
					if ((Object)(object)m_prefab.Asset != (Object)null)
					{
						m_loadedRoom = m_prefab.Asset.GetComponent<Room>();
					}
					else
					{
						Debug.LogError((object)$"Room {m_prefab} wasn't loaded!");
					}
				}
				return m_loadedRoom;
			}
	...
	private void SetupRooms()
	{
		...
		{
			Object.Instantiate<GameObject>(roomList);
		}
	...
	private void GenerateHashList()
	{
		...
		foreach (RoomData room in m_rooms)
		{
			...
			{
				ZLog.LogError((object)("Room with name " + room.m_prefab.Name + " already registered"));
			}
			else
			{
	...
	private void LoadRooms()
	{
		...
		foreach (RoomData room in m_rooms)
		{
			...
			{
				room.m_prefab.Load();
				val.HoldReferenceTo((IReferenceCounted)(object)room.m_prefab);
