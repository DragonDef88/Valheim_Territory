...
public class FishingFloat : MonoBehaviour, IProjectile
{
	...
	public void Setup(Character owner, Vector3 velocity, float hitNoise, HitData hitData, ItemDrop.ItemData item, ItemDrop.ItemData ammo)
	{
		...
		{
			ZNetScene.instance.Destroy(((Component)fishingFloat).gameObject);
		}
		long userID = owner.GetZDOID().UserID;
		m_nview.GetZDO().Set(ZDOVars.s_rodOwner, userID);
		m_nview.GetZDO().Set(ZDOVars.s_bait, ((Object)ammo.m_dropPrefab).name);
		Transform rodTop = GetRodTop(owner);
		if ((Object)(object)rodTop == (Object)null)
		{
			ZLog.LogWarning((object)"Failed to find fishing rod top");
			return;
		}
		m_rodLine.SetPeer(owner.GetZDOID());
		m_lineLength = Vector3.Distance(rodTop.position, ((Component)this).transform.position);
	...
	private Character GetOwner()
	{
		...
		foreach (ZNet.PlayerInfo player in ZNet.instance.GetPlayerList())
		{
			...
			{
				GameObject val = ZNetScene.instance.FindInstance(player.m_characterID);
	...
	{
		Transform val = Utils.FindChild(((Component)owner).transform, "_RodTop", (IterativeSearchType)0);
		if ((Object)(object)val == (Object)null)
		{
			ZLog.LogWarning((object)"Failed to find fishing rod top");
			return null;
	...
	private void FixedUpdate()
	{
		...
		Transform rodTop = GetRodTop(owner);
		if (!Object.op_Implicit((Object)(object)rodTop))
		{
			ZLog.LogWarning((object)"Fishing rod not found, destroying fishing float");
			m_nview.Destroy();
			return;
		...
		Vector3 val = ((Component)rodTop).transform.position - ((Component)this).transform.position;
		float magnitude = ((Vector3)(ref val)).magnitude;
		ItemDrop itemDrop = (Object.op_Implicit((Object)(object)fish) ? ((Component)fish).gameObject.GetComponent<ItemDrop>() : null);
		if (!owner.HaveStamina() && (Object)(object)fish != (Object)null)
		{
			SetCatch(null);
			fish = null;
		...
		if (!Object.op_Implicit((Object)(object)fish) && Utils.LengthXZ(m_body.linearVelocity) > 2f)
		{
			TryToHook();
		}
		if (owner.IsBlocking() && owner.HaveStamina())
		{
			...
			{
				num2 += fish.GetStaminaUse() * (float)(((Object)(object)itemDrop == (Object)null) ? 1 : itemDrop.m_itemData.m_quality);
			}
			...
			if (m_lineLength > magnitude - 0.2f)
			{
				...
				if (Object.op_Implicit((Object)(object)fish) && fish.IsEscaping())
				{
					num3 /= 2f;
				}
				m_lineLength -= fixedDeltaTime * num3;
				m_fishingSkillImproveTimer += fixedDeltaTime * (((Object)(object)fish == (Object)null) ? 1f : m_fishingSkillImproveHookedMultiplier);
		...
		if (magnitude - m_lineLength > m_breakDistance || magnitude > m_maxDistance)
		{
			...
			m_nview.Destroy();
			m_lineBreakEffect.Create(((Component)this).transform.position, Quaternion.identity);
		}
		else
		{
			if (Object.op_Implicit((Object)(object)fish))
			{
				Utils.Pull(m_body, ((Component)fish).transform.position, 0.5f, m_moveForce, 0.5f, 0.3f, false, false, 1f);
			}
			Utils.Pull(m_body, ((Component)rodTop).transform.position, m_lineLength, m_moveForce, 1f, 0.3f, false, false, 1f);
		}
	}
	...
	public static string Catch(Fish fish, Character owner)
	{
		...
		if (!fish.m_extraDrops.IsEmpty())
		{
			foreach (ItemDrop.ItemData dropListItem in fish.m_extraDrops.GetDropListItems())
			{
				...
				{
					ZLog.Log((object)$"picking up {dropListItem.m_stack}x {((Object)dropListItem.m_dropPrefab).name}");
					humanoid.GetInventory().AddItem(dropListItem.m_dropPrefab, dropListItem.m_stack);
				}
				else
				{
					ZLog.Log((object)$"no room, dropping {dropListItem.m_stack}x {((Object)dropListItem.m_dropPrefab).name}");
					Object.Instantiate<GameObject>(dropListItem.m_dropPrefab, ((Component)fish).transform.position, Quaternion.Euler(0f, (float)Random.Range(0, 360), 0f)).GetComponent<ItemDrop>().SetStack(dropListItem.m_stack);
	...
	private void ReturnBait()
	{
		if (!m_baitConsumed)
		{
			...
			if (Object.op_Implicit((Object)(object)prefab) && owner is Player player)
			{
	...
	{
		if ((Object)(object)m_nibbler != (Object)null && Time.time - m_nibbleTime < 0.5f && (Object)(object)GetCatch() == (Object)null)
		{
	...
	public void RPC_Nibble(long sender, ZDOID fishID, bool correctBait)
	{
		...
		if (Time.time - m_nibbleTime < 1f || (Object)(object)GetCatch() != (Object)null)
		{
			return;
		}
		if (correctBait)
		{
			m_nibbleEffect.Create(((Component)this).transform.position, Quaternion.identity, ((Component)this).transform);
			m_body.AddForce(Vector3.down * m_nibbleForce, (ForceMode)2);
			GameObject val = ZNetScene.instance.FindInstance(fishID);
			if (Object.op_Implicit((Object)(object)val))
			{
				m_nibbler = val.GetComponent<Fish>();
				m_nibbleTime = Time.time;
			}
		}
		else
		{
			m_body.AddForce(Vector3.down * m_nibbleForce * 0.5f, (ForceMode)2);
	...
	private static FishingFloat FindFloat(Character owner)
	{
		...
		{
			if ((Object)(object)owner == (Object)(object)allInstance.GetOwner())
			{
				return allInstance;
			}
		}
	...
	public static FishingFloat FindFloat(Fish fish)
	{
		foreach (FishingFloat allInstance in m_allInstances)
		{
			if ((Object)(object)allInstance.GetCatch() == (Object)(object)fish)
			{
				return allInstance;
			}
		}
