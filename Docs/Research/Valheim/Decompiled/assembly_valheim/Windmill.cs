...
public class Windmill : MonoBehaviour
{
	...
	private void Start()
	{
		m_smelter = ((Component)this).GetComponent<Smelter>();
	...
	private void Update()
	{
		...
		m_bom.rotation = Quaternion.RotateTowards(m_bom.rotation, val, m_bomRotationSpeed * powerOutput * Time.deltaTime);
		...
		if ((Object)(object)m_smelter == (Object)null || m_smelter.IsActive())
		{
	...
	private void CheckCover()
	{
		...
		Cover.GetCoverForPoint(((Component)m_propeller).transform.position, ref m_cover, ref flag, 0.5f);
	}
	...
	private void UpdateAudio(float dt)
	{
		...
		{
			obj.volume = Mathf.MoveTowards(obj.volume, num2, m_audioChangeSpeed * dt);
			obj.pitch = Mathf.MoveTowards(obj.pitch, num, m_audioChangeSpeed * dt);
		}
	}
}
