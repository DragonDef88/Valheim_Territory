...
public class ZPlayFabSocket : ZNetStats, IDisposable, ISocket
{
	...
	public ZPlayFabSocket()
	{
		...
		PlayFabMultiplayerManager.Get().LogLevel = (LogLevelType)0;
	}
	...
	public ZPlayFabSocket(string remotePlayerId, Action<PlayFabMatchmakingServerData> serverDataFoundCallback)
	{
		...
		ClientConnect();
		PlayFabMultiplayerManager.Get().OnDataMessageReceived += new OnDataMessageReceivedHandler(OnDataMessageReceived);
		PlayFabMultiplayerManager.Get().OnRemotePlayerJoined += new OnRemotePlayerJoinedHandler(OnRemotePlayerJoined);
		m_isClient = true;
		m_platformPlayerId = ((IUser)PlatformManager.DistributionPlatform.LocalUser).PlatformUserID;
		m_serverDataFoundCallback = serverDataFoundCallback;
		ZPackage zPackage = new ZPackage();
		zPackage.Write((byte)1);
		zPackage.Write(((object)(PlatformUserID)(ref m_platformPlayerId)).ToString());
		Send(zPackage, 64);
		ZLog.Log((object)("PlayFab socket with remote ID " + remotePlayerId + " sent local Platform ID " + GetHostName()));
	}
	...
	private ZPlayFabSocket(PlayFabPlayer remotePlayer)
	{
		...
		PlayFabMultiplayerManager.Get().OnDataMessageReceived += new OnDataMessageReceivedHandler(OnDataMessageReceived);
		ZLog.Log((object)("PlayFab listen socket child connected to remote player " + m_remotePlayerId));
	}
	...
	{
		ZLog.LogWarning((object)("Failed to locate network session for PlayFab player " + m_remotePlayerId));
		switch (failReason)
		{
	...
	private void CheckReestablishConnection(byte[] maybeCompressedBuffer)
	{
		...
		{
			ZLog.Log((object)("Assume restarted game session for remote ID " + GetEndPointString() + " and Platform ID " + GetHostName()));
			ResetAll();
	...
	private void OnDataMessageReceivedCont(byte[] buffer)
	{
		...
		case 64:
			...
			break;
		default:
			ZLog.LogError((object)("Unknown message type " + msgType + " received by socket!\nByte array:\n" + BitConverter.ToString(buffer)));
			return;
		}
		SendAck(++m_next);
	...
	private void InternalReceive(ZPackage pkg)
	{
		...
		if (pkg.ReadByte() == 1)
		{
			...
			ZLog.Log((object)("PlayFab socket with remote ID " + GetEndPointString() + " received local Platform ID " + GetHostName()));
		}
		else
		{
	...
	{
		ZLog.Log((object)("PlayFab client socket to remote player " + m_remotePlayerId + " joined network " + networkId));
	...
	private void OnRemotePlayerJoined(object sender, PlayFabPlayer player)
	{
		...
		{
			ZLog.Log((object)("PlayFab socket connected to remote player " + m_remotePlayerId));
			Connect(player);
		}
	}
	...
	private void Connect(PlayFabPlayer remotePlayer)
	{
		...
		{
			ZLog.Log((object)("Resume TX on " + GetEndPointString()));
		}
		m_peer = (PlayFabPlayer[])(object)new PlayFabPlayer[1] { remotePlayer };
	...
	private void InternalSend(byte[] payload)
	{
		...
		if (m_useCompression)
		{
			if ((Object)(object)ZNet.instance != (Object)null && ZNet.instance.HaveStopped)
			{
	...
	private void InternalSendCont(byte[] compressedPayload)
	{
		...
		if (PlayFabMultiplayerManager.Get().SendDataMessage(compressedPayload, (IEnumerable<PlayFabPlayer>)m_peer, (DeliveryOption)1))
		{
			if (!m_isClient)
			{
		...
		ResetPartyTimeout();
		ZLog.Log((object)("Failed to send, suspend TX on " + GetEndPointString() + " while trying to reconnect"));
	}

	private void ResetPartyTimeout()
	{
		m_partyResetConnectTimeout = Random.Range(9f, 11f) + s_durationToPartyReset;
		m_partyResetTimeout = Random.Range(18f, 22f) + s_durationToPartyReset;
	}
	...
	internal static void ScheduleResetParty()
	{
		...
		{
			s_durationToPartyReset = Random.Range(2.6999998f, 3.3000002f);
		}
	}
	...
	public void Dispose()
	{
		...
		Debug.Log((object)("ZPlayFabSocket::Dispose. State: " + m_state));
		...
		Debug.Log((object)("ZPlayFabSocket::Dispose. leave lobby. LobbyId: " + m_lobbyId));
		if (m_lobbyId != null)
		{
	...
	private void KickstartAfterRecovery()
	{
		...
		{
			ZLog.LogWarning((object)("Failed to resend data on $" + GetEndPointString() + ", closing socket: " + ex.Message));
			Close();
		}
	}
	...
	{
		return ((object)(PlatformUserID)(ref m_platformPlayerId)).ToString();
	}
	...
	internal static void LostConnection(PlayFabPlayer player)
	{
		...
		{
			ZLog.Log((object)("Keep socket for " + value.GetEndPointString() + ", try to reconnect before timeout"));
		}
	}
	...
	internal static void QueueConnection(PlayFabPlayer player)
	{
		...
		if (s_connectSockets.TryGetValue(id, out var value))
		{
			ZLog.Log((object)("Resume TX on " + value.GetEndPointString()));
