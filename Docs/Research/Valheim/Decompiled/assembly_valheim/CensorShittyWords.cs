...
public static class CensorShittyWords
{
	...
	public static bool Filter(string input, out string output, out bool cacheMiss)
	{
		...
		static bool FilterInternal(string input, out string output)
		{
			...
			string text2 = NormalizeStrict(input);
			Dictionary<string, List<int>> dictionary = new Dictionary<string, List<int>>();
			for (int i = 0; i < blacklistDefault.Count; i++)
			{
				int[] array = StringExtensionMethods.AllIndicesOf(text2, blacklistDefaultNormalizedStrict[i]);
				if (array.Length != 0)
				{
			...
			{
				int[] array2 = StringExtensionMethods.AllIndicesOf(text, whitelistDefaultNormalized[k]);
				if (array2.Length != 0)
				{
					Dictionary<string, int[]> dictionary2 = new Dictionary<string, int[]>();
					foreach (KeyValuePair<string, List<int>> item2 in dictionary)
					{
						int[] array3 = StringExtensionMethods.AllIndicesOf(whitelistDefaultNormalizedStrict[k], NormalizeStrict(item2.Key));
						if (array3.Length != 0)
						{
	...
	public static bool Filter(string input, out string output, List<string>[] blacklists, List<string>[] whitelists)
	{
		...
		foreach (List<string> list in array)
		{
			...
			{
				string text3 = NormalizeStrict(list[j]);
				int[] array2 = StringExtensionMethods.AllIndicesOf(text2, text3);
				if (array2.Length == 0)
				{
					continue;
				}
		...
		foreach (List<string> list2 in array)
		{
			...
			{
				string text4 = Normalize(list2[l]);
				int[] array3 = StringExtensionMethods.AllIndicesOf(text, text4);
				if (array3.Length == 0)
				{
					continue;
				}
				string text5 = NormalizeStrict(list2[l]);
				Dictionary<string, int[]> dictionary2 = new Dictionary<string, int[]>();
				foreach (KeyValuePair<string, List<int>> item2 in dictionary)
				{
					int[] array4 = StringExtensionMethods.AllIndicesOf(text5, NormalizeStrict(item2.Key));
					if (array4.Length != 0)
					{
		...
		case UGCFilteringMethod.Censor:
		{
			if (Filter(text, out var output, out var cacheMiss) && cacheMiss)
			{
				...
				case UGCType.ServerName:
					ZLog.LogWarning((object)("Censored server name '" + text + "' -> '" + output + "'"));
					break;
				case UGCType.CharacterName:
					ZLog.LogWarning((object)("Censored character name '" + text + "' -> '" + output + "'"));
					break;
				case UGCType.WorldName:
					ZLog.LogWarning((object)("Censored world name '" + text + "' -> '" + output + "'"));
					break;
				default:
					ZLog.LogWarning((object)("Censored text '" + text + "' -> '" + output + "'"));
					break;
				}
			}
			return output;
	...
	private static UGCFilteringMethod DetermineUGCFilteringMethod(UGCType ugcType, PlatformUserID userId, long playerId, out bool allowAttemptResolve)
	{
		...
		if (!((PlatformUserID)(ref userId)).IsValid && playerId > 0)
		{
		...
		if (!((PlatformUserID)(ref userId)).IsValid)
		{
			PrivilegeResult val = PlatformManager.DistributionPlatform.PrivilegeProvider.CheckPrivilege(GetPrivilegeFromUGCType(ugcType));
		...
		if (((IUser)PlatformManager.DistributionPlatform.LocalUser).PlatformUserID == userId)
		{
			allowAttemptResolve = false;
			return UGCFilteringMethod.None;
		}
		PrivilegeResult val2 = PlatformManager.DistributionPlatform.PrivilegeProvider.CheckPrivilege((Privilege)2);
		allowAttemptResolve = true;
		if ((int)val2 != 0)
		{
			...
			IUserProfile val3 = default(IUserProfile);
			if (!PlatformManager.DistributionPlatform.RelationsProvider.TryGetUserProfile(userId, ref val3))
			{
				PlatformManager.DistributionPlatform.RelationsProvider.GetUserProfileAsync(userId, (GetUserProfileCompletedHandler)null, (GetUserProfileFailedHandler)null);
				return UGCFilteringMethod.Block;
			}
			if ((int)val3.CheckPermission(GetPermissionFromUGCType(ugcType)) != 0)
			{
				return UGCFilteringMethod.Block;
			}
		}
	...
	private static void TryShowUGCNotification(bool allowAttemptResolve)
	{
		...
		{
			if (!((UIController)resolvePrivilege).IsOpen)
			{
				resolvePrivilege.Open((Privilege)2, (PrivilegeResult)64);
				((UIController)resolvePrivilege).Closed += new UIClosedHandler(OnResolvePrivilegeUIClosed);
			}
		}
