...
public class ConnectPanel : MonoBehaviour
{
	...
	private void Start()
	{
		...
		Rect rect = m_playerList.rect;
		m_playerListBaseSize = ((Rect)(ref rect)).height;
	}
	...
	public static bool IsVisible()
	{
		...
		{
			return ((Component)m_instance.m_root).gameObject.activeSelf;
		}
		return false;
	}
	...
	private void Update()
	{
		...
		if (ZInput.GetKeyDown((KeyCode)283, true) || (ZInput.GetButton("JoyLTrigger") && ZInput.GetButton("JoyLBumper") && ZInput.GetButtonDown("JoyBack")))
		{
			((Component)m_root).gameObject.SetActive(!((Component)m_root).gameObject.activeSelf);
		}
		if (!((Component)m_root).gameObject.activeInHierarchy)
		{
			return;
		}
		if (!ZNet.instance.IsServer() && ZNet.GetConnectionStatus() == ZNet.ConnectionStatus.Connected)
		{
			((Component)m_serverField).gameObject.SetActive(true);
			m_serverField.text = ZNet.GetServerString();
		}
		else
		{
			((Component)m_serverField).gameObject.SetActive(false);
		}
		...
		((Component)m_myPort).gameObject.SetActive(ZNet.instance.IsServer());
		...
		if (playerList.Count != m_playerListElements.Count)
		{
			...
			{
				Object.Destroy((Object)(object)playerListElement);
			}
			m_playerListElements.Clear();
			for (int i = 0; i < playerList.Count; i++)
			{
				GameObject val = Object.Instantiate<GameObject>(m_playerElement, (Transform)(object)m_playerList);
				Transform transform = val.transform;
				((RectTransform)((transform is RectTransform) ? transform : null)).anchoredPosition = new Vector2(0f, (float)i * (0f - num));
				m_playerListElements.Add(val);
			}
			float num2 = (float)playerList.Count * num;
			num2 = Mathf.Max(m_playerListBaseSize, num2);
			m_playerList.SetSizeWithCurrentAnchors((Axis)1, num2);
			m_playerListScroll.value = 1f;
		}
		for (int j = 0; j < playerList.Count; j++)
		{
			ZNet.PlayerInfo playerInfo = playerList[j];
			TMP_Text component = ((Component)m_playerListElements[j].transform.Find("name")).GetComponent<TMP_Text>();
			TMP_Text component2 = ((Component)m_playerListElements[j].transform.Find("hostname")).GetComponent<TMP_Text>();
			Button component3 = ((Component)m_playerListElements[j].transform.Find("KickButton")).GetComponent<Button>();
			component.text = CensorShittyWords.FilterUGC(playerInfo.m_name, UGCType.CharacterName, playerInfo.m_userInfo.m_id, 0L);
			component2.text = ((object)(PlatformUserID)(ref playerInfo.m_userInfo.m_id)).ToString();
			((Component)component3).gameObject.SetActive(false);
		}
		((Selectable)m_connectButton).interactable = ValidHost();
	}
	...
	private bool ValidHost()
	{
		...
		try
		{
			num = int.Parse(((TMP_InputField)m_hostPort).text);
		}
		catch
		{
			return false;
		}
		if (string.IsNullOrEmpty(((TMP_InputField)m_hostName).text) || num == 0)
		{
			return false;
		}
		return true;
