...
public class ZDOMan
{
	...
	public void PrepareSave()
	{
		...
		ZLog.Log((object)("PrepareSave: clone done in " + stopwatch.ElapsedMilliseconds + "ms"));
		stopwatch = Stopwatch.StartNew();
		ZDOExtraData.PrepareSave();
		ZLog.Log((object)("PrepareSave: ZDOExtraData.PrepareSave done in " + stopwatch.ElapsedMilliseconds + " ms"));
	}
	...
	private void WarnAndRemoveBrokenZDOs(List<ZDO> warningZDOs, List<ZDO> brokenZDOs, int totalNumZDOs, int numZDOs)
	{
		...
		if (warningZDOs.Count > 0)
		{
			...
			ZLog.LogWarning((object)("Found " + value + " ZDOs with unknown prefabs. Will load anyway."));
			...
			foreach (KeyValuePair<int, int> item in dictionary)
			{
				...
				int num = key;
				int num2 = value;
				ZLog.LogWarning((object)("    Hash " + num + " appeared " + num2 + " times."));
			}
		}
		...
		obj[6] = " ZDOs loaded.";
		ZLog.LogError((object)string.Concat(obj));
		...
		foreach (KeyValuePair<int, int> item2 in dictionary2)
		{
			...
			int num3 = key;
			int num4 = value;
			ZLog.LogError((object)("    Hash " + num3 + " filtered out " + num4 + " times."));
		}
	}
	...
	private void RPC_ZDOData(ZRpc rpc, ZPackage pkg)
	{
		...
		while (true)
		{
			...
			{
				zDO = CreateNewZDO(zDOID, val);
				flag = true;
			}
			zDO.OwnerRevision = num3;
			zDO.DataRevision = num4;
			zDO.SetOwnerInternal(ownerInternal);
			zDO.InternalSetPosition(val);
	...
	private void RemoveOrphanNonPersistentZDOS()
	{
		foreach (KeyValuePair<ZDOID, ZDO> item in m_objectsByID)
		{
			...
			if (!value.Persistent && (!value.HasOwner() || !IsPeerConnected(value.GetOwner())))
			{
				...
				ZLog.Log((object)("Destroying abandoned non persistent zdo " + uid.ToString() + " owner " + value.GetOwner()));
	...
	private void ConvertCreationTime(List<ZDO> zdos)
	{
		...
		{
			StringExtensionMethods.GetStableHashCode("cultivate"),
			StringExtensionMethods.GetStableHashCode("raise"),
			StringExtensionMethods.GetStableHashCode("path"),
			StringExtensionMethods.GetStableHashCode("paved_road"),
			StringExtensionMethods.GetStableHashCode("HeathRockPillar"),
			StringExtensionMethods.GetStableHashCode("HeathRockPillar_frac"),
			StringExtensionMethods.GetStableHashCode("ship_construction"),
			StringExtensionMethods.GetStableHashCode("replant"),
			StringExtensionMethods.GetStableHashCode("digg"),
			StringExtensionMethods.GetStableHashCode("mud_road"),
			StringExtensionMethods.GetStableHashCode("LevelTerrain"),
			StringExtensionMethods.GetStableHashCode("digg_v2")
		};
		int num = 0;
		foreach (ZDO zdo in zdos)
		{
		...
		if (num > 0)
		{
			Debug.Log((object)("Converted " + num + " Creation Times."));
		}
	}

	private void ConvertPortals()
	{
		Debug.Log((object)("ConvertPortals => Make sure all " + m_portalObjects.Count + " portals are in a good state."));
		int num = 0;
		...
		if (num > 0)
		{
			Debug.Log((object)("ConvertPortals => fixed " + num + " portals."));
		}
	}
	...
	private void ConnectPortals()
	{
		...
		if (num > 0)
		{
			Debug.Log((object)("ConnectPortals => Connected " + num + " portals."));
		}
	}

	private void ConvertSpawners()
	{
		List<ZDOID> allZDOIDsWithHash = ZDOExtraData.GetAllZDOIDsWithHash(ZDOExtraData.Type.Long, StringExtensionMethods.GetStableHashCode("spawn_id_u"));
		if (allZDOIDsWithHash.Count > 0)
		{
			Debug.Log((object)("ConvertSpawners => Will try and convert " + allZDOIDsWithHash.Count + " spawners."));
		}
		int num = 0;
		int num2 = 0;
		...
		if (num > 0 || num2 > 0)
		{
			Debug.Log((object)("ConvertSpawners => Converted " + num + " spawners, and " + num2 + " 'done' spawners."));
		}
	}
	...
	private void ConnectSpawners()
	{
		...
		if (num > 0 || num2 > 0)
		{
			Debug.Log((object)("ConnectSpawners => Connected " + num + " spawners and " + num2 + " 'done' spawners."));
		}
	}

	private void ConvertSyncTransforms()
	{
		List<ZDOID> allZDOIDsWithHash = ZDOExtraData.GetAllZDOIDsWithHash(ZDOExtraData.Type.Long, StringExtensionMethods.GetStableHashCode("parentID_u"));
		if (allZDOIDsWithHash.Count > 0)
		{
			Debug.Log((object)("ConvertSyncTransforms => Will try and convert " + allZDOIDsWithHash.Count + " SyncTransforms."));
		}
		int num = 0;
		...
		if (num > 0)
		{
			Debug.Log((object)("ConvertSyncTransforms => Converted " + num + " SyncTransforms."));
		}
	}
	...
	private void ConvertSeed()
	{
		...
		if (num > 0)
		{
			Debug.Log((object)("ConvertSeed => Converted " + num + " ZDOs."));
		}
	}
		...
		foreach (ZDO zdo in zdos)
		{
			...
			if (@int != 0)
			{
				...
				for (int i = 0; i < @int; i++)
				{
					...
					Utils.Write(binaryWriter, vec);
					Utils.Write(binaryWriter, quaternion);
				}
				zdo.Set(ZDOVars.s_roomData, memoryStream.ToArray());
				ZLog.Log((object)$"Cleaned up old dungeon data format for {@int} rooms.");
			}
		}
	}
	...
	private void ConnectSyncTransforms()
	{
		...
		if (num > 0)
		{
			Debug.Log((object)("ConnectSyncTransforms => Connected " + num + " SyncTransforms."));
		}
	}
}
