...
public class ZSteamSocketOLD : IDisposable, ISocket
{
	...
	private static void OnConnectionFailed(P2PSessionConnectFail_t data)
	{
		...
		ZLog.Log((object)("Got connection failed callback: " + ((object)(CSteamID)(ref steamIDRemote)).ToString()));
	...
	private static void OnSessionRequest(P2PSessionRequest_t data)
	{
		...
		CSteamID steamIDRemote = data.m_steamIDRemote;
		ZLog.Log((object)("Got session request from " + ((object)(CSteamID)(ref steamIDRemote)).ToString()));
	...
	private static void QueueNewPkg(CSteamID sender, byte[] data)
	{
		...
		{
			val = sender;
			ZLog.Log((object)("Got package from unconnected peer " + ((object)(CSteamID)(ref val)).ToString()));
			listner.QueuePendingConnection(sender).QueuePackage(data);
		}
		else
		{
			val = sender;
			ZLog.Log((object)("Got package from unkown peer " + ((object)(CSteamID)(ref val)).ToString() + " but no active listner"));
		}
	}
