...
public class WearNTear : MonoBehaviour, IDestructible
{
	...
	private static readonly RaycastHit[] s_raycastHits = (RaycastHit[])(object)new RaycastHit[128];

	private static readonly Collider[] s_tempColliders = (Collider[])(object)new Collider[128];
	...
	private void Awake()
	{
		...
		if (m_nview.GetZDO() != null)
		{
			...
			if (s_rayMask == 0)
			{
				s_rayMask = LayerMask.GetMask(new string[5] { "piece", "Default", "static_solid", "Default_small", "terrain" });
			}
			...
			{
				num = Random.Range(0.1f * m_health, m_health * 0.6f);
			...
			m_updateCoverTimer = Random.Range(0f, 4f);
	...
	private void Start()
	{
		...
		m_connectedHeightMap = Heightmap.FindHeightmap(((Component)this).transform.position);
		if ((Object)(object)m_connectedHeightMap != (Object)null)
		{
	...
	private void SetAshlandsMaterialValue(float v)
	{
		if (m_ashMaterialValue != v)
		{
			...
			MaterialMan.instance.SetValue(((Component)this).gameObject, s_AshlandsDamageShaderID, v);
		}
	}
	...
	public void Highlight()
	{
		...
		((Color)(ref val))._002Ector(0.6f, 0.8f, 1f);
		if (supportColorValue >= 0f)
		{
			val = Color.Lerp(new Color(1f, 0f, 0f), new Color(0f, 1f, 0f), supportColorValue);
		...
		MaterialMan.instance.SetValue<Color>(((Component)this).gameObject, ShaderProps._EmissionColor, val * 0.4f);
		MaterialMan.instance.SetValue<Color>(((Component)this).gameObject, ShaderProps._Color, val);
		((MonoBehaviour)this).CancelInvoke("ResetHighlight");
		((MonoBehaviour)this).Invoke("ResetHighlight", 0.2f);
	}
	...
	private void SetupColliders()
	{
		...
		m_colliders = ((Component)this).GetComponentsInChildren<Collider>(true);
		m_bounds = new List<BoundData>();
		Collider[] colliders = m_colliders;
		foreach (Collider val in colliders)
		{
			if (!val.isTrigger && !((Object)(object)val.attachedRigidbody != (Object)null))
			{
				BoundData item = default(BoundData);
				if (val is BoxCollider)
				{
					BoxCollider val2 = (BoxCollider)(object)((val is BoxCollider) ? val : null);
					item.m_rot = ((Component)val2).transform.rotation;
					item.m_pos = ((Component)val2).transform.position + ((Component)val2).transform.TransformVector(val2.center);
					item.m_size = new Vector3(((Component)val2).transform.lossyScale.x * val2.size.x, ((Component)val2).transform.lossyScale.y * val2.size.y, ((Component)val2).transform.lossyScale.z * val2.size.z);
				}
				else
				{
					item.m_rot = Quaternion.identity;
					Bounds bounds = val.bounds;
					item.m_pos = ((Bounds)(ref bounds)).center;
					bounds = val.bounds;
					item.m_size = ((Bounds)(ref bounds)).size;
				}
				item.m_size.x += 0.3f;
				item.m_size.y += 0.3f;
				item.m_size.z += 0.3f;
				ref Vector3 size = ref item.m_size;
				size *= 0.5f;
				m_bounds.Add(item);
			}
		}
	...
	public void UpdateWear(float time)
	{
		...
		{
			if (ZNetScene.instance.OutsideActiveArea(((Component)this).transform.position))
			{
				m_support = GetMaxSupport();
				m_nview.GetZDO().Set(ZDOVars.s_support, m_support);
				return;
			}
			bool flag = ShieldGenerator.IsInsideShieldCached(((Component)this).transform.position, ref m_shieldChangeID);
			float num = 0f;
			m_rainWet = !flag && !m_haveRoof && EnvMan.IsWet();
			if (Object.op_Implicit((Object)(object)m_wet))
			{
			...
			bool flag2 = false;
			if (m_biome == Heightmap.Biome.None || !Object.op_Implicit((Object)(object)m_heightmap))
			{
				...
				{
					m_biome = m_heightmap.GetBiome(((Component)this).transform.position);
					float num2 = 9999f;
					...
					if (m_staticPosition)
					{
						m_lavaValue = m_heightmap.GetLava(((Component)this).transform.position);
					}
				}
			}
	...
	private Vector3 GetCOM()
	{
		...
		return ((Component)this).transform.position + ((Component)this).transform.rotation * m_comOffset;
	}
	...
	private void UpdateSupport()
	{
		...
		if (count > 0)
		{
			...
			{
				Collider val = m_supportColliders[i];
				if ((Object)(object)val == (Object)null)
				{
					break;
				}
				WearNTear componentInParent = ((Component)val).GetComponentInParent<WearNTear>();
				if ((Object)(object)componentInParent == (Object)null || !componentInParent.m_supports)
				{
					break;
				}
				if ((Object)(object)val != (Object)null && ((Component)val).transform.position == m_supportPositions[i])
				{
		...
		foreach (BoundData bound in m_bounds)
		{
			...
			if (m_clearCachedSupport)
			{
				...
				{
					Collider val2 = s_tempColliders[j];
					if ((Object)(object)val2.attachedRigidbody != (Object)null || val2.isTrigger || m_colliders.Contains(val2))
					{
						continue;
					}
					WearNTear componentInParent2 = ((Component)val2).GetComponentInParent<WearNTear>();
			...
			{
				Collider val3 = s_tempColliders[k];
				if ((Object)(object)val3.attachedRigidbody != (Object)null || val3.isTrigger || m_colliders.Contains(val3))
				{
					continue;
				}
				if (((Component)val3).gameObject.layer == s_terrainLayer)
				{
					flag = true;
					continue;
				}
				WearNTear componentInParent3 = ((Component)val3).GetComponentInParent<WearNTear>();
				...
				float num6 = Vector3.Distance(cOM, ((Component)componentInParent3).transform.position) + 0.1f;
				...
				Vector3 val4 = FindSupportPoint(cOM, componentInParent3, val3);
				if (val4.y < cOM.y + 0.05f)
				{
					Vector3 val5 = val4 - cOM;
					Vector3 normalized = ((Vector3)(ref val5)).normalized;
					if (normalized.y < 0f)
					{
						float num7 = Mathf.Acos(1f - Mathf.Abs(normalized.y)) / ((float)Math.PI / 2f);
						float num8 = Mathf.Lerp(horizontalLoss, verticalLoss, num7);
						float num9 = support2 - num8 * num5 * support2;
						num3 = Mathf.Max(num3, num9);
					}
					float item = support2 - verticalLoss * num5 * support2;
					s_tempSupportPoints.Add(val4);
					s_tempSupportPointValues.Add(item);
					m_supportColliders.Add(val3);
					m_supportPositions.Add(((Component)val3).transform.position);
		...
		if (s_tempSupportPoints.Count > 0)
		{
			...
			{
				Vector3 val6 = s_tempSupportPoints[l] - cOM;
				val6.y = 0f;
				for (int m = l + 1; m < count2; m++)
				{
					...
					if (!(num10 <= num3))
					{
						Vector3 val7 = s_tempSupportPoints[m] - cOM;
						val7.y = 0f;
						if (Vector3.Angle(val6, val7) >= 100f)
						{
							num3 = num10;
						}
					}
	...
	private static Vector3 FindSupportPoint(Vector3 com, WearNTear wnt, Collider otherCollider)
	{
		...
		//IL_0034: Unknown result type (might be due to invalid IL or missing references)
		MeshCollider val = (MeshCollider)(object)((otherCollider is MeshCollider) ? otherCollider : null);
	...
	{
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
		return Floating.IsUnderWater(((Component)this).transform.position, ref m_previousWaterVolume);
	}
	...
	private bool HaveRoof()
	{
		...
		return RoofCheck(((Component)this).transform.position, out m_roof);
	}
	...
	public static bool RoofCheck(Vector3 position, out GameObject roofObject)
	{
		...
		for (int i = 0; i < num; i++)
		{
			RaycastHit val = s_raycastHits[i];
			if (!((Component)((RaycastHit)(ref val)).collider).gameObject.CompareTag("leaky"))
			{
				roofObject = ((Component)((RaycastHit)(ref val)).collider).gameObject;
				return true;
			}
		}
		roofObject = null;
	...
	private bool HaveAshRoof()
	{
		...
		//IL_0048: Unknown result type (might be due to invalid IL or missing references)
		if (Object.op_Implicit((Object)(object)m_ashroof))
		{
			return true;
		}
		int num = Physics.SphereCastNonAlloc(((Component)this).transform.position, 0.1f, Vector3.up, s_raycastHits, 100f, s_rayMask);
		for (int i = 0; i < num; i++)
		{
			RaycastHit val = s_raycastHits[i];
			if ((Object)(object)((Component)((RaycastHit)(ref val)).collider).gameObject != (Object)(object)((Component)this).gameObject && ((Object)(object)((Component)((RaycastHit)(ref val)).collider).transform.parent == (Object)null || (Object)(object)((Component)((Component)((RaycastHit)(ref val)).collider).transform.parent).gameObject != (Object)(object)((Component)this).gameObject))
			{
				m_ashroof = ((Component)((RaycastHit)(ref val)).collider).gameObject;
				return true;
			}
		}
		return false;
	...
	private void SetHealthVisual(float health, bool triggerEffects)
	{
		...
		if ((Object)(object)m_worn == (Object)null && (Object)(object)m_broken == (Object)null && (Object)(object)m_new == (Object)null)
		{
			return;
		}
		if (health > 0.75f)
		{
			if ((Object)(object)m_worn != (Object)(object)m_new)
			{
				m_worn.SetActive(false);
			}
			if ((Object)(object)m_broken != (Object)(object)m_new)
			{
		...
		else if (health > 0.25f)
		{
			...
			{
				m_switchEffect.Create(((Component)this).transform.position, ((Component)this).transform.rotation, ((Component)this).transform);
			}
			if ((Object)(object)m_new != (Object)(object)m_worn)
			{
				m_new.SetActive(false);
			}
			if ((Object)(object)m_broken != (Object)(object)m_worn)
			{
				m_broken.SetActive(false);
			}
		...
		else
		{
			...
			if ((Object)(object)m_new != (Object)(object)m_broken)
			{
				m_new.SetActive(false);
			}
			if ((Object)(object)m_worn != (Object)(object)m_broken)
			{
				m_worn.SetActive(false);
			}
	...
	public bool ApplyDamage(float damage, HitData hitData = null)
	{
		...
		{
			Terminal.Log(string.Format("Damage WNT: {0} took {1} damage from {2}", ((Object)((Component)this).gameObject).name, damage, (hitData == null) ? ((object)"UNKNOWN") : ((object)hitData)));
		}
		if (@float <= 0f)
		{
			Destroy(hitData);
	...
	private void Destroy(HitData hitData = null, bool blockDrop = false)
	{
		...
		Bed component = ((Component)this).GetComponent<Bed>();
		if ((Object)(object)component != (Object)null && m_nview.IsOwner() && (Object)(object)Game.instance != (Object)null)
		{
		...
		m_health = 0f;
		ClearCachedSupport();
		if (Object.op_Implicit((Object)(object)m_piece) && !blockDrop)
		{
	...
	private void RPC_CreateFragments(long peer)
	{
		...
		{
			Destructible.CreateFragments(((Component)this).gameObject);
		}
	}
