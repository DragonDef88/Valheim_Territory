...
public class Fermenter : MonoBehaviour, Hoverable, Interactable
{
	...
	private void Awake()
	{
		...
		m_fermentingObject.SetActive(false);
		...
		if (!((Object)(object)m_nview == (Object)null) && m_nview.GetZDO() != null)
		{
			...
			{
				((MonoBehaviour)this).InvokeRepeating("SlowUpdate", 2f, 2f);
			}
			else
			{
				((MonoBehaviour)this).InvokeRepeating("SlowUpdate", 0f, 2f);
			}
	...
	private bool AddItem(Humanoid user, ItemDrop.ItemData item)
	{
		...
		m_nview.InvokeRPC("RPC_AddItem", ((Object)item.m_dropPrefab).name);
		return true;
	}
	...
	private void RPC_AddItem(long sender, string name)
	{
		...
		if (m_nview.IsOwner() && GetStatus() == Status.Empty)
		{
			...
			m_addedEffects.Create(((Component)this).transform.position, ((Component)this).transform.rotation);
	...
	private void RPC_Tap(long sender)
	{
		...
		if (m_nview.IsOwner() && GetStatus() == Status.Ready)
		{
			...
			((MonoBehaviour)this).Invoke("DelayedTap", m_tapDelay);
			m_tapEffects.Create(((Component)this).transform.position, ((Component)this).transform.rotation);
			m_nview.GetZDO().Set(ZDOVars.s_content, "");
			m_nview.GetZDO().Set(ZDOVars.s_startTime, 0);
		}
	}
	...
	private void DelayedTap()
	{
		...
		m_spawnEffects.Create(((Component)m_outputPoint).transform.position, Quaternion.identity);
		...
		if (itemConversion != null)
		{
			...
			for (int i = 0; i < itemConversion.m_producedItems; i++)
			{
				Vector3 val = m_outputPoint.position + Vector3.up * num;
				ItemDrop.OnCreateNew(Object.Instantiate<ItemDrop>(itemConversion.m_to, val, Quaternion.identity));
			}
		}
	}
	...
	private void UpdateCover(float dt, bool forceUpdate = false)
	{
		...
		if (m_updateCoverTimer <= 0f || forceUpdate)
		{
			...
			bool hasRoof = default(bool);
			Cover.GetCoverForPoint(m_roofCheckPoint.position, ref num, ref hasRoof, 0.5f);
			m_exposed = num < 0.7f;
			m_hasRoof = hasRoof;
