...
public class DropTable
{
	...
	public List<ItemDrop.ItemData> GetDropListItems()
	{
		...
		int num2 = Random.Range(m_dropMin, m_dropMax + 1);
	...
	private void AddItemToList(List<ItemDrop.ItemData> toDrop, DropData data)
	{
		...
		itemData2.m_stack = (data.m_dontScale ? Random.Range(num, num2 + 1) : Game.instance.ScaleDrops(itemData2, num, num2 + 1));
	...
	{
		int amount = Random.Range(m_dropMin, m_dropMax + 1);
	...
	private List<GameObject> GetDropList(int amount)
	{
		...
		for (int i = 0; (float)i < num; i++)
		{
			...
			foreach (DropData item in dropsTemp)
			{
				...
				{
					ZLog.LogWarning((object)$"Droptable item '{item.m_item}' has a weight of 0 and will not be dropped correctly!");
				}
			}
			...
			for (int j = 0; j < amount; j++)
			{
				...
				foreach (DropData item2 in dropsTemp)
				{
					...
					if (num6 <= num7)
					{
						flag = true;
						int num8 = 0;
						num8 = ((!item2.m_dontScale) ? ((int)Mathf.Max(1f, Mathf.Round(Random.Range(Mathf.Round((float)item2.m_stackMin * num4), Mathf.Round((float)item2.m_stackMax * num4))))) : ((i == 0) ? Random.Range(item2.m_stackMin, item2.m_stackMax) : 0));
