...
public class ObjectDB : MonoBehaviour
{
	...
	private void UpdateRegisters()
	{
		...
		{
			m_itemByHash.Add(StringExtensionMethods.GetStableHashCode(((Object)item).name), item);
	...
	{
		return GetItemPrefab(StringExtensionMethods.GetStableHashCode(name));
	}
	...
	{
		return TryGetItemPrefab(StringExtensionMethods.GetStableHashCode(name), out prefab);
	}
	...
	{
		return StringExtensionMethods.GetStableHashCode(((Object)prefab).name);
	}
	...
	public List<ItemDrop> GetAllItems(ItemDrop.ItemData.ItemType type, string startWith)
	{
		...
		{
			ItemDrop component = item.GetComponent<ItemDrop>();
			if (component.m_itemData.m_shared.m_itemType == type && Utils.CustomStartsWith(((Object)((Component)component).gameObject).name, startWith))
			{
				list.Add(component);
			}
		}
	...
	public Recipe GetRecipe(ItemDrop.ItemData item)
	{
		...
		{
			if (!((Object)(object)recipe.m_item == (Object)null) && recipe.m_item.m_itemData.m_shared.m_name == item.m_shared.m_name)
			{
				return recipe;
			}
		}
