...
public class HotkeyBar : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Object.op_Implicit((Object)(object)localPlayer) && !InventoryGui.IsVisible() && !Menu.IsVisible() && !GameCamera.InFreeFly() && !Minimap.IsOpen() && !Hud.IsPieceSelectionVisible() && !StoreGui.IsVisible() && !Console.IsVisible() && !Chat.instance.HasFocus() && !PlayerCustomizaton.IsBarberGuiVisible() && !Hud.InRadial())
		{
	...
	private void UpdateIcons(Player player)
	{
		...
		if (!Object.op_Implicit((Object)(object)player) || player.IsDead())
		{
		...
		for (int j = 0; j < m_items.Count; j++)
		{
			...
			((Component)elementData2.m_durability).gameObject.SetActive(flag2);
			if (flag2)
			{
	...
	private void ToggleBindingHint(bool bShouldEnable)
	{
		...
		{
			TMP_Text component = ((Component)m_elements[i].m_go.transform.Find("binding")).GetComponent<TMP_Text>();
			if (bShouldEnable)
			{
				component.text = (i + 1).ToString();
			}
