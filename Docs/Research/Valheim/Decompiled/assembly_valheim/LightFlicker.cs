...
public class LightFlicker : MonoBehaviour, IMonoUpdater
{
	...
	public void CustomUpdate(float deltaTime, float time)
	{
		...
		if (!Object.op_Implicit((Object)(object)m_light))
		{
			ZLog.LogError((object)"Light was null! This should never happen!");
		}
		else
		{
			if (!((Behaviour)m_light).enabled)
			{
				return;
			}
			m_time += deltaTime;
			...
			if (m_ttl > 0f)
			{
				...
				float num2 = m_ttl - m_fadeDuration;
				m_targetIntensity *= 1f - Utils.LerpStep(num2, m_ttl, m_time);
			}
			if (m_smoothFlicker)
			{
				float num3 = ((m_time > m_ttl - m_fadeDuration && m_ttl > 0f) ? 0.075f : 0.15f);
				m_smoothedIntensity = Utils.LerpSmooth(m_smoothedIntensity, m_targetIntensity, deltaTime, num3);
			}
			else
			{
			...
			m_offset *= m_movement;
			((Component)this).transform.localPosition = m_basePosition + m_offset;
		}
	}
}
