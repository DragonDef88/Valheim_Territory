...
public class DistantFogEmitter : MonoBehaviour
{
	...
	private void PlaceOne()
	{
		...
		if (GetRandomPoint(((Component)this).transform.position, out var p))
		{
			EmitParams val = default(EmitParams);
			((EmitParams)(ref val)).position = p + Vector3.up * m_placeOffset;
			m_psystems[Random.Range(0, m_psystems.Length)].Emit(val, 1);
		}
	}
	...
	private bool GetRandomPoint(Vector3 center, out Vector3 p)
	{
		...
		float num2 = Mathf.Sqrt(Random.value) * (m_maxRadius - m_minRadius) + m_minRadius;
		...
		if (p.y < 30f)
		{
			...
			if (Random.value > m_waterSpawnChance)
			{
				return false;
			}
			p.y = 30f;
		...
		{
			if (Random.value > m_mountainSpawnChance)
			{
				return false;
			}
		}
		else if (Random.value > m_landSpawnChance)
		{
			return false;
		}
		return true;
