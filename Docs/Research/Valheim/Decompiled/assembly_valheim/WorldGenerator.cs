...
public class WorldGenerator
{
	...
	private WorldGenerator(World world)
	{
		...
		Random.InitState(m_world.m_seed);
		if (m_noiseGen == null)
		{
		...
		m_offset0 = Random.Range(-10000, 10000);
	...
	private void FindLakes()
	{
		...
		for (float num = -10000f; num <= 10000f; num = (float)((double)num + 128.0))
		{
			...
			{
				Vector2 val = new Vector2(num2, num);
				if (!(((Vector2)(ref val)).magnitude > 10000f) && GetBaseHeight(num2, num, menuTerrain: false) < 0.05f)
				{
	...
	private List<Vector2> MergePoints(List<Vector2> points, float range)
	{
		...
		while (points.Count > 0)
		{
			...
			while (points.Count > 0)
			{
				int num = FindClosest(points, val, range);
				if (num == -1)
				{
					break;
				}
				val = (val + points[num]) * 0.5f;
	...
	private List<River> PlaceStreams()
	{
		...
		for (int i = 0; i < 3000; i++)
		{
			...
			{
				Vector2 val = (p + end) * 0.5f;
				float pregenerationHeight = GetPregenerationHeight(val.x, val.y);
	...
	private bool FindStreamEndPoint(int iterations, float minHeight, float maxHeight, Vector2 start, float minLength, float maxLength, out Vector2 end)
	{
		...
		for (int i = 0; i < iterations; i++)
		{
			...
			Vector2 val = start + new Vector2(Mathf.Sin(num3), Mathf.Cos(num3)) * num2;
			float pregenerationHeight = GetPregenerationHeight(val.x, val.y);
	...
	private bool FindStreamStartPoint(int iterations, float minHeight, float maxHeight, out Vector2 p, out float starth)
	{
		...
		for (int i = 0; i < iterations; i++)
		{
			float num = Random.Range(-10000f, 10000f);
			float num2 = Random.Range(-10000f, 10000f);
	...
	private List<River> PlaceRivers()
	{
		...
		{
			Vector2 val = list2[0];
			int num = FindRandomRiverEnd(list, m_lakes, val, 2000f, 0.4f, 128f);
			if (num == -1 && !HaveRiver(list, val))
			{
				num = FindRandomRiverEnd(list, m_lakes, val, 5000f, 0.4f, 128f);
			}
			if (num != -1)
			{
				River river = new River();
				...
				river.widthMax = Random.Range(60f, 100f);
				river.widthMin = Random.Range(60f, river.widthMax);
				float num2 = Vector2.Distance(river.p0, river.p1);
				river.curveWidth = (float)((double)num2 / 15.0);
				river.curveWavelength = (float)((double)num2 / 20.0);
				list.Add(river);
	...
	private bool IsRiverAllowed(Vector2 p0, Vector2 p1, float step, float heightLimit)
	{
		...
		{
			Vector2 val2 = p0 + normalized * num2;
			float baseHeight = GetBaseHeight(val2.x, val2.y, menuTerrain: false);
	...
	private void RenderRivers(List<River> rivers)
	{
		...
		foreach (River river in rivers)
		{
			...
			Vector2 val = river.p1 - river.p0;
			Vector2 normalized = ((Vector2)(ref val)).normalized;
			((Vector2)(ref val2))._002Ector(0f - normalized.y, normalized.x);
			float num2 = Vector2.Distance(river.p0, river.p1);
			for (float num3 = 0f; num3 <= num2; num3 = (float)((double)num3 + (double)num))
			{
				...
				float r = Random.Range(river.widthMin, river.widthMax);
				Vector2 p = river.p0 + normalized * num3 + val2 * num5;
	...
	public bool InsideRiverGrid(Vector2i grid, Vector2 p, float r)
	{
		...
		((Vector2)(ref val))._002Ector((float)((double)grid.x * 64.0), (float)((double)grid.y * 64.0));
		Vector2 val2 = p - val;
		if (Math.Abs(val2.x) < (float)((double)r + 32.0))
		{
			return Math.Abs(val2.y) < (float)((double)r + 32.0);
		}
		return false;
	}
	...
	private void GetWeight(RiverPoint[] points, float wx, float wy, out float weight, out float width)
	{
		...
		//IL_003a: Unknown result type (might be due to invalid IL or missing references)
		Vector2 val = default(Vector2);
		((Vector2)(ref val))._002Ector(wx, wy);
		weight = 0f;
		width = 0f;
		float num = 0f;
		float num2 = 0f;
		for (int i = 0; i < points.Length; i++)
		{
			...
			float num3 = Vector2.SqrMagnitude(riverPoint.p - val);
	...
	public static bool IsDeepnorth(float x, float y)
	{
		...
		Vector2 val = new Vector2(x, (float)((double)y + 4000.0));
		return ((Vector2)(ref val)).magnitude > (float)(12000.0 + (double)num);
	}
	...
	public void GetTerrainDelta(Vector3 center, float radius, out float delta, out Vector3 slopeDirection)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			float height = GetHeight(val4.x, val4.z);
			if (height < num3)
			{
				num3 = height;
				val2 = val4;
