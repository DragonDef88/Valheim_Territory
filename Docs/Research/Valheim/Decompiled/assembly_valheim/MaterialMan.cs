...
public class MaterialMan : MonoBehaviour
{
	private class PropertyContainer
	{
		...
		public PropertyContainer(GameObject go, MaterialPropertyBlock block)
		{
			...
			{
				m_assignedRenderers.AddRange((IEnumerable<Renderer>)(object)componentsInChildren);
			}
			SkinnedMeshRenderer[] componentsInChildren2 = go.GetComponentsInChildren<SkinnedMeshRenderer>(true);
			if (componentsInChildren2 != null)
			{
				m_assignedRenderers.AddRange((IEnumerable<Renderer>)(object)componentsInChildren2);
			}
		...
		public void UpdateBlock()
		{
			...
			foreach (KeyValuePair<int, ShaderPropertyBase> shaderProperty in m_shaderProperties)
			{
				...
				{
					m_propertyBlock.SetVector(shaderProperty.Key, Vector4.op_Implicit((shaderProperty.Value as ShaderProperty<Vector3>).Get()));
				}
				else if (shaderProperty.Value.PropertyType == typeof(Vector2))
				{
					m_propertyBlock.SetVector(shaderProperty.Key, Vector4.op_Implicit((shaderProperty.Value as ShaderProperty<Vector2>).Get()));
				}
			...
			{
				if ((Object)(object)m_assignedRenderers[num] == (Object)null)
				{
	...
	{
		if (!m_blocks.ContainsKey(((Object)gameObject).GetInstanceID()))
		{
			...
			m_blocks.Add(((Object)gameObject).GetInstanceID(), propertyContainer);
		}
	}

	public void UnregisterRenderers(GameObject gameObject)
	{
		if (!m_blocks.TryGetValue(((Object)gameObject).GetInstanceID(), out var value))
		{
			ZLog.LogError((object)("Can't unregister renderer for " + ((Object)gameObject).name));
			return;
		}
		...
		m_blocks.Remove(((Object)gameObject).GetInstanceID());
	}
	...
	public void SetValue<T>(GameObject go, int nameID, T value)
	{
		...
		m_blocks[((Object)go).GetInstanceID()].SetValue(nameID, value);
	}

	public void ResetValue(GameObject go, int nameID)
	{
		if (m_blocks.ContainsKey(((Object)go).GetInstanceID()))
		{
			m_blocks[((Object)go).GetInstanceID()].ResetValue(nameID);
		}
	}
}
