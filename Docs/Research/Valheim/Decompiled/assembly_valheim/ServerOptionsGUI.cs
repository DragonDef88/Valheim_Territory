...
public class ServerOptionsGUI : MonoBehaviour
{
	...
	{
		m_instance = this;
		m_modifiers = ((Component)m_modifiersRoot.transform).GetComponentsInChildren<KeyUI>();
		m_presets = ((Component)m_presetsRoot.transform).GetComponentsInChildren<KeyButton>();
	}
	...
	private void Update()
	{
		...
		((Component)m_toolTipPanel).gameObject.SetActive(m_toolTipText.text.Length > 0);
	...
	public static void Initizalize()
	{
		...
		for (int i = 0; i < modifiers.Length; i++)
		{
			...
			{
				ZLog.LogError((object)$"Modifier {keySlider.m_nameLabel} is setup without a defined modifier");
			}
			...
			foreach (KeySlider.SliderSetting setting in keySlider.m_settings)
			{
				...
				{
					ZLog.LogError((object)$"Modifier setting {setting.m_name} in {keySlider.m_nameLabel} is setup without a modifier option");
				}
	...
	public static string GetWorldModifierSummary(IEnumerable<string> keys, bool alwaysShort = false, string divider = ", ")
	{
		...
		if (m_presets == null)
		{
			ZLog.LogWarning((object)"Can't get world modifier summary until prefab has been initiated.");
			return "Error!";
		}
		...
		if ((Object)(object)keyButton != (Object)null)
		{
	...
	public static bool TryConvertModifierKeysToCompactKVP<T>(ICollection<string> keys, out T result) where T : IDictionary<string, string>, new()
	{
		...
		foreach (string key in keys)
		{
			...
			{
				ZLog.LogError((object)("Failed to parse key " + key + " as GlobalKeys!"));
				return false;
			}
			int num5;
			...
			{
				string text3 = "";
				int[] array = StringExtensionMethods.AllIndicesOf(text2, ':');
				for (int i = 0; i < array.Length + 1; i++)
				{
					...
					if (num3 >= num4)
					{
						ZLog.LogError((object)("Failed to parse value " + text2 + "'s subkey as WorldModifiers and WorldModifierOption: separator index in wrong location!"));
					}
					if (num3 < 0)
					{
						...
						{
							ZLog.LogError((object)("Failed to parse value " + text2 + "'s subvalue " + text4 + " as WorldPresets: Value enum couldn't be parsed!"));
							return false;
						}
						string text5 = text3;
						num5 = (int)result3;
					...
					{
						ZLog.LogError((object)("Failed to parse value " + text2 + "'s subkey " + text6 + " as WorldModifiers: Key enum couldn't be parsed!"));
						return false;
					}
					if (!Enum.TryParse<WorldModifierOption>(text7, ignoreCase: true, out var result5) || result5.ToString().ToLower() != text7.ToLower())
					{
						ZLog.LogError((object)("Failed to parse value " + text2 + "'s subvalue " + text7 + " as WorldModifierOption: Value enum couldn't be parsed!"));
						return false;
					}
					string text8 = text3;
					num5 = (int)result4;
	...
	public static bool TryConvertCompactKVPToModifierKeys(IDictionary<string, string> kvps, out string[] result)
	{
		...
		foreach (KeyValuePair<string, string> kvp in kvps)
		{
			...
			{
				ZLog.LogError((object)("Failed to parse key " + kvp.Key + " as GlobalKeys: " + kvp.Key + " could not be parsed as an integer!"));
				return false;
			}
			if (!Enum.IsDefined(typeof(GlobalKeys), result2))
			{
				ZLog.LogError((object)string.Format("Failed to parse key {0} as {1}: {2} is out of range!", kvp.Key, "GlobalKeys", result2));
			}
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				string text2 = "";
				int[] array3 = StringExtensionMethods.AllIndicesOf(array2[i], ':');
				for (int j = 0; j < array3.Length + 1; j++)
				{
					...
					if (num3 >= num4)
					{
						ZLog.LogError((object)("Failed to parse value " + text + "'s subkey as WorldModifiers and WorldModifierOption: separator index in wrong location!"));
					}
					if (num3 < 0)
					{
						...
						{
							ZLog.LogError((object)("Failed to parse value " + text3 + " as WorldPresets: " + text3 + " could not be parsed as an integer!"));
							return false;
						}
						if (!Enum.IsDefined(typeof(WorldPresets), result3))
						{
							ZLog.LogError((object)string.Format("Failed to parse value {0} as {1}: {2} is out of range!", text3, "WorldPresets", result3));
						}
						string text4 = text2;
					...
					{
						ZLog.LogError((object)("Failed to parse value " + text5 + " as WorldModifiers: " + text5 + " could not be parsed as an integer!"));
						return false;
					}
					if (!Enum.IsDefined(typeof(WorldModifiers), result4))
					{
						ZLog.LogError((object)string.Format("Failed to parse value {0} as {1}: {2} is out of range!", text5, "WorldModifiers", result4));
					}
					if (!int.TryParse(text6, out var result5))
					{
						ZLog.LogError((object)("Failed to parse value " + text6 + " as WorldModifierOption: " + text6 + " could not be parsed as an integer!"));
						return false;
					}
					if (!Enum.IsDefined(typeof(WorldModifierOption), result5))
					{
						ZLog.LogError((object)string.Format("Failed to parse value {0} as {1}: {2} is out of range!", text6, "WorldModifierOption", result5));
					}
					string text7 = text2;
