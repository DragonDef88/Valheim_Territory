...
public class Aoe : MonoBehaviour, IProjectile, IMonoUpdater
{
	...
	private static Collider[] s_hits = (Collider[])(object)new Collider[100];
	...
	private void Awake()
	{
		...
		if (m_hitCharacters)
		{
			m_rayMask |= LayerMask.GetMask(new string[3] { "character", "character_net", "character_ghost" });
		}
		if (m_hitProps)
		{
			m_rayMask |= LayerMask.GetMask(new string[7] { "Default", "static_solid", "Default_small", "piece", "hitbox", "character_noenv", "vehicle" });
		}
		if (m_hitTerrain)
		{
			m_rayMask |= LayerMask.GetMask(new string[1] { "terrain" });
		}
		...
		{
			m_statusEffectIfBossHash = StringExtensionMethods.GetStableHashCode(m_statusEffectIfBoss);
		}
		if (!string.IsNullOrEmpty(m_statusEffectIfPlayer))
		{
			m_statusEffectIfPlayerHash = StringExtensionMethods.GetStableHashCode(m_statusEffectIfPlayer);
		}
	...
	public void CustomFixedUpdate(float fixedDeltaTime)
	{
		...
		if ((Object)(object)m_owner != (Object)null && m_attachToCaster)
		{
			((Component)this).transform.position = ((Component)m_owner).transform.TransformPoint(m_offset);
			((Component)this).transform.rotation = ((Component)m_owner).transform.rotation * m_localRot;
		}
		if (m_activationTimer > 0f)
		{
			return;
		...
		if (m_chainStartChance > 0f && m_chainDelay >= 0f)
		{
			...
			if (m_chainDelay <= 0f && Random.value < m_chainStartChance)
			{
				...
				foreach (Collider s_hit in s_hitList)
				{
					...
					{
						Vector3 position2 = ((Component)s_hit).gameObject.transform.position;
						bool flag = false;
						for (int i = 0; i < s_chainObjs.Count; i++)
						{
							if (Object.op_Implicit((Object)(object)s_chainObjs[i]))
							{
						...
						if (!flag)
						{
							GameObject val = Object.Instantiate<GameObject>(m_chainObj, position2, ((Component)s_hit).gameObject.transform.rotation);
							s_chainObjs.Add(val);
							IProjectile componentInChildren = val.GetComponentInChildren<IProjectile>();
							if (componentInChildren != null)
							{
								componentInChildren.Setup(m_owner, VectorExtensions.DirTo(position, position2), -1f, m_hitData, m_itemData, m_ammo);
							...
							num--;
							float num2 = Vector3.Distance(position2, ((Component)this).transform.position);
							GameObject[] array = m_chainEffects.Create(position + Vector3.up, Quaternion.LookRotation(VectorExtensions.DirTo(position, position2 + Vector3.up)));
	...
	public void Initiate()
	{
		...
		m_initiateEffect.Create(((Component)this).transform.position, Quaternion.identity);
		CheckHits();
	}
	...
	private void CheckHits()
	{
		...
		if (m_maxTargetsFromCenter > 0)
		{
			...
			int num = m_maxTargetsFromCenter;
			{
				foreach (Collider s_hit in s_hitList)
				{
					if (OnHit(s_hit, ((Component)s_hit).transform.position))
					{
						num--;
					}
					if (num <= 0)
		...
		{
			OnHit(s_hitList[i], ((Component)s_hitList[i]).transform.position);
		}
	}
	...
	private void FindHits()
	{
		...
		m_hitList.Clear();
		int num = (((Object)(object)m_useCollider != (Object)null) ? Physics.OverlapBoxNonAlloc(((Component)this).transform.position + m_useCollider.center, m_useCollider.size / 2f, s_hits, ((Component)this).transform.rotation, m_rayMask) : Physics.OverlapSphereNonAlloc(((Component)this).transform.position, m_radius, s_hits, m_rayMask));
		s_hitList.Clear();
		for (int i = 0; i < num; i++)
		{
	...
	private bool ShouldHit(Collider collider)
	{
		...
		if (Object.op_Implicit((Object)(object)val))
		{
			...
			if (component != null)
			{
				if ((Object)(object)m_nview == (Object)null && !component.IsOwner())
				{
					return false;
				}
				if ((Object)(object)m_owner != (Object)null)
				{
					if (!m_hitOwner && (Object)(object)component == (Object)(object)m_owner)
					{
						return false;
					}
					...
					bool flag = BaseAI.IsEnemy(m_owner, component) || (Object.op_Implicit((Object)(object)component.GetBaseAI()) && component.GetBaseAI().IsAggravatable() && m_owner.IsPlayer());
	...
	{
		s_hitList.Sort((Collider a, Collider b) => Vector3.Distance(((Component)a).transform.position, ((Component)this).transform.position).CompareTo(Vector3.Distance(((Component)b).transform.position, ((Component)this).transform.position)));
	}
	...
	public void Setup(Character owner, Vector3 velocity, float hitNoise, HitData hitData, ItemDrop.ItemData item, ItemDrop.ItemData ammo)
	{
		...
		if (m_attachToCaster && (Object)(object)owner != (Object)null)
		{
			m_offset = ((Component)owner).transform.InverseTransformPoint(((Component)this).transform.position);
			m_localRot = Quaternion.Inverse(((Component)owner).transform.rotation) * ((Component)this).transform.rotation;
		}
	...
	private void CauseTriggerDamage(Collider collider, bool onTriggerEnter)
	{
		...
		if (!(m_triggerEnterOnly && onTriggerEnter) && !(m_activationTimer > 0f))
		{
			if (!m_useTriggers)
			{
				ZLog.LogWarning((object)("AOE got OnTriggerStay but trigger damage is disabled in " + ((Object)((Component)this).gameObject).name));
			}
			else if (ShouldHit(collider))
			{
				Bounds bounds = collider.bounds;
				OnHit(collider, ((Bounds)(ref bounds)).center);
			}
		}
	}
	...
	private bool OnHit(Collider collider, Vector3 hitPoint)
	{
		...
		GameObject val = Projectile.FindHitObject(collider);
		if (m_hitList.Contains(val))
		{
			return false;
		}
		m_hitList.Add(val);
		float num = 1f;
		if (Object.op_Implicit((Object)(object)m_owner) && m_owner.IsPlayer() && m_skill != 0)
		{
		...
		{
			num2 = m_distanceScaleCurve.Evaluate(Mathf.Clamp01(Vector3.Distance(val.transform.position, ((Component)this).transform.position) / m_radius));
		}
		...
		if (component != null)
		{
			if (!m_hitParent)
			{
				if (!((Object)(object)((Component)this).gameObject.transform.parent != (Object)null) || !((Object)(object)val == (Object)(object)((Component)((Component)this).gameObject.transform.parent).gameObject))
				{
					IDestructible componentInParent = ((Component)this).gameObject.GetComponentInParent<IDestructible>();
		...
		Heightmap component2 = val.GetComponent<Heightmap>();
		if (component2 != null)
		{
			FootStep.GroundMaterial groundMaterial = component2.GetGroundMaterial(Vector3.up, ((Component)this).transform.position, m_groundLavaValue);
			FootStep.GroundMaterial groundMaterial2 = component2.GetGroundMaterial(Vector3.up, ((Component)this).transform.position);
			FootStep.GroundMaterial groundMaterial3 = ((m_groundLavaValue >= 0f) ? groundMaterial : groundMaterial2);
			if (Object.op_Implicit((Object)(object)m_spawnOnHitTerrain) && (m_spawnOnGroundType == FootStep.GroundMaterial.Everything || m_spawnOnGroundType.HasFlag(groundMaterial3)) && (!m_hitTerrainOnlyOnce || !m_hasHitTerrain))
			{
				...
				{
					GameObject val3 = Attack.SpawnOnHitTerrain(val2, m_spawnOnHitTerrain, m_owner, m_hitNoise, null, null, m_randomRotation);
					...
					val2 += new Vector3(insideUnitCircle.x * num5, 0f, insideUnitCircle.y * num5);
					if (Object.op_Implicit((Object)(object)val3) && i > 0)
					{
						val3.transform.localScale = Utils.Vec3((1f - num4) * (m_multiSpawnScaleMax - m_multiSpawnScaleMin) + m_multiSpawnScaleMin);
					}
					if (m_multiSpawnSpringDelayMax > 0f)
					{
						ConditionalObject componentInChildren = val3.GetComponentInChildren<ConditionalObject>();
						if (componentInChildren != null)
						{
					...
					if (m_placeOnGround)
					{
						val3.transform.position = new Vector3(val3.transform.position.x, ZoneSystem.instance.GetGroundHeight(val3.transform.position), val3.transform.position.z);
					}
				}
			}
			result = true;
		...
		if (Object.op_Implicit((Object)(object)character))
		{
			...
			if (m_launchCharacters)
			{
				...
				num6 *= num2;
				Vector3 val4 = VectorExtensions.DirTo(hitPoint, ((Component)this).transform.position);
				if (m_launchForceUpFactor > 0f)
				{
					val4 = Vector3.Slerp(val4, Vector3.up, m_launchForceUpFactor);
				}
				character.ForceJump(((Vector3)(ref val4)).normalized * num6);
			}
			flag = true;
		}
		else if (!m_hitProps)
		...
		bool flag2 = (component is Destructible destructible && (Object)(object)destructible.m_spawnWhenDestroyed != (Object)null) || val.GetComponent<MineRock5>() != null;
		Vector3 val6;
		if (!m_attackForceForward)
		{
			Vector3 val5 = hitPoint - ((Component)this).transform.position;
		...
		hitData.m_point = (flag2 ? ((Component)this).transform.position : hitPoint);
		hitData.m_dir = dir;
		...
		hitData.m_ignorePVP = (Object)(object)m_owner == (Object)(object)character || m_ignorePVP;
		...
		{
			Terminal.Log("Damage AOE: hitting target" + (((Object)(object)m_owner == (Object)null) ? " without owner" : (" with owner: " + (object)m_owner)));
		}
		if (m_damageSelf > 0f)
		{
			IDestructible componentInParent2 = ((Component)this).GetComponentInParent<IDestructible>();
		...
		goto IL_065a;
		IL_065a:
		if (val.GetComponent<MineRock5>() == null)
		{
			m_hitEffects.Create(hitPoint, Quaternion.identity);
		}
		if (!m_gaveSkill && Object.op_Implicit((Object)(object)m_owner) && m_skill != Skills.SkillType.None && flag && m_canRaiseSkill)
		{
