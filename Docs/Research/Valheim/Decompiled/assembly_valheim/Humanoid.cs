...
public class Humanoid : Character
{
	...
	public void GiveDefaultItems()
	{
		...
		{
			GameObject val = m_randomShield[Random.Range(0, m_randomShield.Length)];
		...
		{
			GameObject val2 = m_randomWeapon[Random.Range(0, m_randomWeapon.Length)];
			if (Object.op_Implicit((Object)(object)val2))
			{
				GiveDefaultItem(val2);
			}
		}
		if (m_randomArmor.Length != 0)
		{
			GameObject val3 = m_randomArmor[Random.Range(0, m_randomArmor.Length)];
			if (Object.op_Implicit((Object)(object)val3))
			{
				GiveDefaultItem(val3);
			}
		}
		if (m_randomSets.Length != 0)
		{
			defaultItems = m_randomSets[Random.Range(0, m_randomSets.Length)].m_items;
		...
		if (m_randomItems.Length != 0)
		{
			...
			{
				if (Object.op_Implicit((Object)(object)randomItem.m_prefab) && Random.value > randomItem.m_chance)
				{
	...
	private void StartAttackGroundCheck()
	{
		...
		{
			int layer = ((Component)lastGroundCollider).gameObject.layer;
	...
	private void DrainEquipedItemDurability(ItemDrop.ItemData item, float dt)
	{
		...
		{
			Message(MessageHud.MessageType.TopLeft, Localization.instance.Localize("$msg_broke", new string[1] { item.m_shared.m_name }), 0, item.GetIcon());
	...
	public ItemDrop.ItemData GetCurrentWeapon()
	{
		...
		if (Object.op_Implicit((Object)(object)m_unarmedWeapon))
		{
	...
	public override void OnAttackTrigger()
	{
		if (m_nview.IsValid() && m_nview.IsOwner() && m_currentAttack != null && GetCurrentWeapon() != null)
		{
			((MonoBehaviour)this).StartCoroutine(EndAttackGroundCheck());
	...
	public ItemDrop.ItemData PickupPrefab(GameObject prefab, int stackSize = 0, bool autoequip = true)
	{
		...
		if (Pickup(val, autoequip))
		{
			return val.GetComponent<ItemDrop>().m_itemData;
		}
	...
	public bool Pickup(GameObject go, bool autoequip = true, bool autoPickupDelay = true)
	{
		...
		m_pickupEffects.Create(((Component)this).transform.position, Quaternion.identity);
		if (IsPlayer())
		{
			ShowPickupMessage(component.m_itemData, stack);
			if ((Object)(object)Player.m_localPlayer == (Object)(object)(this as Player) && Hud.instance.m_radialMenu.Active)
			{
	...
	public void EquipBestWeapon(Character targetCreature, StaticTarget targetStatic, Character hurtFriend, Character friend)
	{
		...
		if (Object.op_Implicit((Object)(object)targetCreature))
		{
			...
			num = Vector3.Distance(((Component)targetCreature).transform.position, ((Component)this).transform.position) - radius;
		}
		else if (Object.op_Implicit((Object)(object)targetStatic))
		{
			num = Vector3.Distance(((Component)targetStatic).transform.position, ((Component)this).transform.position);
		}
		...
		foreach (ItemDrop.ItemData item in allItems)
		{
			...
			else if (item.m_shared.m_aiTargetType == ItemDrop.ItemData.AiTarget.Friend && !((Object)(object)friend == (Object)null) && !(time - item.m_lastAttackTime < item.m_shared.m_aiAttackInterval))
			{
				if (item.m_shared.m_aiPrioritized)
				{
					EquipItem(item);
		...
		if (optimalWeapons.Count > 0)
		{
			...
			EquipItem(optimalWeapons[Random.Range(0, optimalWeapons.Count)]);
		}
		else if (outofRangeWeapons.Count > 0)
		{
			...
			EquipItem(outofRangeWeapons[Random.Range(0, outofRangeWeapons.Count)]);
		}
		else if (allWeapons.Count > 0)
		{
			...
			EquipItem(allWeapons[Random.Range(0, allWeapons.Count)]);
		}
		else
		{
			ItemDrop.ItemData currentWeapon = GetCurrentWeapon();
	...
	public bool DropItem(Inventory inventory, ItemDrop.ItemData item, int amount)
	{
		...
		ItemDrop itemDrop = ItemDrop.DropItem(item, amount, ((Component)this).transform.position + ((Component)this).transform.forward + ((Component)this).transform.up, ((Component)this).transform.rotation);
		if (IsPlayer())
		{
		...
		((Component)itemDrop).GetComponent<Rigidbody>().linearVelocity = (((Component)this).transform.forward + Vector3.up) * num;
		m_zanim.SetTrigger("interact");
		m_dropEffects.Create(((Component)this).transform.position, Quaternion.identity);
	...
	public void UseItem(Inventory inventory, ItemDrop.ItemData item, bool fromInventoryGui)
	{
		...
		if (item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Consumable)
		{
			...
			{
				m_consumeItemEffects.Create(((Component)Player.m_localPlayer).transform.position, Quaternion.identity);
		...
		else
		{
			...
			if (!fromInventoryGui && m_useItemBlockMessage == 0)
			{
				if (hoverable != null)
				{
					Message(MessageHud.MessageType.Center, Localization.instance.Localize("$msg_cantuseon", new string[2]
					{
				...
				{
					Message(MessageHud.MessageType.Center, Localization.instance.Localize("$msg_useonwhat", new string[1] { item.m_shared.m_name }));
				}
			}
	...
	protected void DoInteractAnimation(GameObject obj)
	{
		...
		Vector3 val = obj.transform.position - ((Component)this).transform.position;
		val.y = 0f;
	...
	{
		if (Object.op_Implicit((Object)(object)m_nview) && m_nview.IsValid() && Object.op_Implicit((Object)(object)m_visEquipment))
		{
			...
			m_visEquipment.SetRightItemVisual(Utils.GetPrefabName(((Object)obj).name));
		}
	}
	...
	public bool EquipItem(ItemDrop.ItemData item, bool triggerEquipEffects = true)
	{
		...
		if (item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Tool)
		{
			...
			m_hiddenRightItem = null;
			m_hiddenLeftItem = null;
			if (Object.op_Implicit((Object)(object)m_visEquipment) && m_visEquipment.m_isPlayer)
			{
		...
		else if (item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Chest)
		{
			...
			if (Object.op_Implicit((Object)(object)m_visEquipment) && m_visEquipment.m_isPlayer)
			{
				item.m_shared.m_equipEffect.Create(((Component)this).transform.position + Vector3.up, ((Component)this).transform.rotation);
			}
		}
		else if (item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Legs)
		{
			...
			if (Object.op_Implicit((Object)(object)m_visEquipment) && m_visEquipment.m_isPlayer)
			{
				item.m_shared.m_equipEffect.Create(((Component)this).transform.position, ((Component)this).transform.rotation);
			}
		}
	...
	public void UnequipItem(ItemDrop.ItemData item, bool triggerEquipEffects = true)
	{
		...
		SetupEquipment();
		item.m_shared.m_unequipEffect.Create(((Component)this).transform.position, Quaternion.identity);
		if (triggerEquipEffects)
		{
			TriggerEquipEffect(item);
		}
	...
	private void TriggerEquipEffect(ItemDrop.ItemData item)
	{
		...
		if (m_nview.GetZDO() != null && MonoUpdaters.UpdateCount != m_lastEquipEffectFrame)
		{
			...
			m_equipEffects.Create(((Component)this).transform.position, Quaternion.identity);
		}
	}
	...
	{
		VisEquipment component = ((Component)ragdoll).GetComponent<VisEquipment>();
	...
	protected virtual void SetupVisEquipment(VisEquipment visEq, bool isRagdoll)
	{
		if (!isRagdoll)
		{
			visEq.SetLeftItem((m_leftItem != null) ? ((Object)m_leftItem.m_dropPrefab).name : "", (m_leftItem != null) ? m_leftItem.m_variant : 0);
			visEq.SetRightItem((m_rightItem != null) ? ((Object)m_rightItem.m_dropPrefab).name : "");
			if (IsPlayer())
			{
				visEq.SetLeftBackItem((m_hiddenLeftItem != null) ? ((Object)m_hiddenLeftItem.m_dropPrefab).name : "", (m_hiddenLeftItem != null) ? m_hiddenLeftItem.m_variant : 0);
				visEq.SetRightBackItem((m_hiddenRightItem != null) ? ((Object)m_hiddenRightItem.m_dropPrefab).name : "");
			}
		}
		visEq.SetChestItem((m_chestItem != null) ? ((Object)m_chestItem.m_dropPrefab).name : "");
		visEq.SetLegItem((m_legItem != null) ? ((Object)m_legItem.m_dropPrefab).name : "");
		visEq.SetHelmetItem((m_helmetItem != null) ? ((Object)m_helmetItem.m_dropPrefab).name : "");
		visEq.SetShoulderItem((m_shoulderItem != null) ? ((Object)m_shoulderItem.m_dropPrefab).name : "", (m_shoulderItem != null) ? m_shoulderItem.m_variant : 0);
		visEq.SetUtilityItem((m_utilityItem != null) ? ((Object)m_utilityItem.m_dropPrefab).name : "");
		visEq.SetTrinketItem((m_trinketItem != null) ? ((Object)m_trinketItem.m_dropPrefab).name : "");
		if (IsPlayer())
		{
	...
	{
		if (Object.op_Implicit((Object)(object)m_visEquipment) && (m_nview.GetZDO() == null || m_nview.IsOwner()))
		{
			SetupVisEquipment(m_visEquipment, isRagdoll: false);
		}
		if (m_nview.GetZDO() != null)
		{
			...
			if (m_rightItem != null && Object.op_Implicit((Object)(object)m_rightItem.m_shared.m_buildPieces))
			{
	...
	private void SetupAnimationState()
	{
		...
		else if ((Object)(object)m_unarmedWeapon != (Object)null)
		{
	...
	private void UpdateEquipmentStatusEffects()
	{
		...
		if (m_leftItem != null && Object.op_Implicit((Object)(object)m_leftItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_leftItem.m_shared.m_equipStatusEffect);
		}
		if (m_rightItem != null && Object.op_Implicit((Object)(object)m_rightItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_rightItem.m_shared.m_equipStatusEffect);
		}
		if (m_chestItem != null && Object.op_Implicit((Object)(object)m_chestItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_chestItem.m_shared.m_equipStatusEffect);
		}
		if (m_legItem != null && Object.op_Implicit((Object)(object)m_legItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_legItem.m_shared.m_equipStatusEffect);
		}
		if (m_helmetItem != null && Object.op_Implicit((Object)(object)m_helmetItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_helmetItem.m_shared.m_equipStatusEffect);
		}
		if (m_shoulderItem != null && Object.op_Implicit((Object)(object)m_shoulderItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_shoulderItem.m_shared.m_equipStatusEffect);
		}
		if (m_utilityItem != null && Object.op_Implicit((Object)(object)m_utilityItem.m_shared.m_equipStatusEffect))
		{
			hashSet.Add(m_utilityItem.m_shared.m_equipStatusEffect);
		}
		if (m_trinketItem != null && Object.op_Implicit((Object)(object)m_trinketItem.m_shared.m_equipStatusEffect))
		{
	...
	private bool HaveSetEffect(ItemDrop.ItemData item)
	{
		...
		if ((Object)(object)item.m_shared.m_setStatusEffect == (Object)null || item.m_shared.m_setName.Length == 0 || item.m_shared.m_setSize <= 1)
		{
			return false;
		}
	...
	protected override bool BlockAttack(HitData hit, Character attacker)
	{
		...
		if (Vector3.Dot(hit.m_dir, ((Component)this).transform.forward) > 0f)
		{
			return false;
		}
		...
		if (currentBlocker.m_shared.m_buildBlockCharges)
		{
			...
			currentBlocker.m_shared.m_blockChargeEffects.Create(m_visEquipment.m_leftHand.position, ((Component)this).transform.rotation, null, 1f, m_blockCharges);
		...
		if (Object.op_Implicit((Object)(object)attacker) && !flag)
		{
			AddAdrenaline(currentBlocker.m_shared.m_blockAdrenaline);
		}
		if (Object.op_Implicit((Object)(object)attacker) && flag && flag3)
		{
			...
			if (Object.op_Implicit((Object)(object)currentBlocker.m_shared.m_perfectBlockStatusEffect))
			{
			...
			else if (Object.op_Implicit((Object)(object)m_perfectBlockStatusEffect))
			{
		...
		if (flag3)
		{
			...
			if (Object.op_Implicit((Object)(object)attacker) && !hit.m_ranged)
			{
				float num6 = 1f - Mathf.Clamp01(num2 * 0.5f);
				HitData hitData = new HitData();
				hitData.m_pushForce = currentBlocker.GetDeflectionForce() * num6;
				hitData.m_dir = ((Component)attacker).transform.position - ((Component)this).transform.position;
				hitData.m_dir.y = 0f;
				((Vector3)(ref hitData.m_dir)).Normalize();
