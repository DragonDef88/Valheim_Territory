...
public class Inventory
{
	...
	public bool AddItem(GameObject prefab, int amount)
	{
		...
		ZLog.Log((object)("adding " + ((Object)prefab).name + "  " + itemData.m_stack));
	...
	public void GetAllPieceTables(List<PieceTable> tables)
	{
		foreach (ItemDrop.ItemData item in m_inventory)
		{
			if ((Object)(object)item.m_shared.m_buildPieces != (Object)null && !tables.Contains(item.m_shared.m_buildPieces))
			{
	...
	public ItemDrop.ItemData GetItem(string name, int quality = -1, bool isPrefabName = false)
	{
		foreach (ItemDrop.ItemData item in m_inventory)
		{
			if (((isPrefabName && ((Object)item.m_dropPrefab).name == name) || (!isPrefabName && item.m_shared.m_name == name)) && (quality < 0 || quality == item.m_quality) && item.m_worldLevel >= Game.m_worldLevel)
			{
				return item;
			}
		}
	...
	public ItemDrop.ItemData GetAmmoItem(string ammoName, string matchPrefabName = null)
	{
		...
		foreach (ItemDrop.ItemData item in m_inventory)
		{
			if ((item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Ammo || item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.AmmoNonEquipable || item.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Consumable) && item.m_shared.m_ammoType == ammoName && (matchPrefabName == null || ((Object)item.m_dropPrefab).name == matchPrefabName))
			{
	...
	public void Print()
	{
		for (int i = 0; i < m_inventory.Count; i++)
		{
			...
			ZLog.Log((object)(i + ": " + itemData.m_shared.m_name + "  " + itemData.m_stack + " / " + itemData.m_shared.m_maxStackSize));
		}
	}
	...
	public void Save(ZPackage pkg)
	{
		...
		foreach (ItemDrop.ItemData item in m_inventory)
		{
			...
			{
				pkg.Write(((Object)item.m_dropPrefab).name);
			}
	...
	public List<ItemDrop.ItemData> GetHotbar(bool includeEmpty = false)
	{
		...
		List<ItemDrop.ItemData> list = new List<ItemDrop.ItemData>();
		List<ItemDrop.ItemData> source = m_inventory.Where((ItemDrop.ItemData i) => i.m_gridPos.y == 0).ToList();
		int j;
		for (j = 0; j < 8; j++)
		{
			list.Add(source.FirstOrDefault((ItemDrop.ItemData item) => item.m_gridPos.x == j));
		}
		return list;
	}
