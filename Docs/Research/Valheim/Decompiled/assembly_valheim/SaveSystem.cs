...
public class SaveSystem
{
	...
	public static bool TryGetSaveByName(string name, SaveDataType dataType, out SaveWithBackups save)
	{
		...
		if (!s_saveCollections.TryGetValue(dataType, out var value))
		{
			ZLog.LogError((object)$"Failed to retrieve collection of type {dataType}!");
			save = null;
			return false;
		}
	...
	public static string GetSavePath(SaveDataType dataType, FileSource source)
	{
		...
		default:
			ZLog.LogError((object)$"Reload not implemented for save data type {dataType}!");
			return null;
		}
	}
	...
	public static bool Copy(SaveFile file, string newName, FileSource destinationLocation = 0)
	{
		...
		for (int i = 0; i < allPaths.Length; i++)
		{
			...
			{
				ZLog.LogError((object)("Failed to get save info for file " + allPaths[i]));
				return false;
			}
			if (!TryConvertSource(allPaths[i], file.m_source, destinationLocation, out var destinationPath))
			{
				ZLog.LogError((object)$"Failed to convert source from {file.m_source} to {destinationLocation} for file {allPaths[i]}");
				return false;
			}
	...
	public static bool MoveSource(SaveFile file, bool isBackup, FileSource destinationSource, out bool cloudQuotaExceeded)
	{
		...
		for (int i = 0; i < allPaths.Length; i++)
		{
			...
			{
				ZLog.LogError((object)$"Failed to convert source from {file.m_source} to {destinationSource} for file {allPaths[i]}");
				return false;
			}
		}
		if ((int)destinationSource == 2 && FileHelpers.OperationExceedsCloudCapacity(file.Size))
		{
			ZLog.LogWarning((object)"This operation would exceed the cloud save quota and has therefore been aborted!");
		...
		if (flag)
		{
			ZLog.LogError((object)"Copying world into cloud failed, aborting move to cloud.");
			for (int k = 0; k < num; k++)
			{
				FileHelpers.Delete(array[k], (FileSource)2);
			}
			InvalidateCache();
			return false;
		}
		file.ParentSaveWithBackups.AddSaveFile(array, destinationSource);
		if ((int)file.m_source != 2 && !isBackup)
		{
	...
	public static RestoreBackupResult RestoreMetaFromMostRecentBackup(SaveFile saveFile)
	{
		...
		{
			ZLog.LogError((object)("Caught exception while restoring meta from backup: " + ex.ToString()));
			InvalidateCache();
			return RestoreBackupResult.UnknownError;
		}
	...
	public static RestoreBackupResult RestoreBackup(SaveFile backup)
	{
		...
		else
		{
			...
			flag = (int)backup.m_source == 1 && saveFileType == SaveFileType.CloudBackup;
		}
		if (!Copy(backup, newName, (FileSource)(flag ? 2 : ((int)backup.m_source))))
		{
			return RestoreBackupResult.CopyFailed;
		}
		return RestoreBackupResult.Success;
	...
	public static bool CheckMove(string saveName, SaveDataType dataType, ref FileSource source, DateTime now, ulong opUsage = 0uL, bool copyToNewLocation = false)
	{
		//IL_001b: Unknown result type (might be due to invalid IL or missing references)
		//IL_0022: Invalid comparison between Unknown and I4
		SaveFile saveFile = null;
		if (TryGetSaveByName(saveName, dataType, out var save) && !save.IsDeleted && (int)save.PrimaryFile.m_source == (int)source)
		{
		...
		if ((int)source == 1 && FileHelpers.CloudStorageEnabled && !FileHelpers.LocalStorageSupported && !FileHelpers.OperationExceedsCloudCapacity(opUsage))
		{
			source = (FileSource)2;
			if (saveFile != null)
			{
	...
	public static bool CreateBackup(SaveFile saveFile, DateTime now, FileSource source = 0)
	{
		//IL_0022: Unknown result type (might be due to invalid IL or missing references)
	...
	public static bool ConsiderBackup(string saveName, SaveDataType dataType, DateTime now, int backupCount, int backupShort, int backupLong, int waitFirstBackup, float worldTime = 0f)
	{
		...
		ZLog.Log((object)$"Considering autobackup. World time: {worldTime}, short time: {backupShort}, long time: {backupLong}, backup count: {backupCount}");
		if (worldTime > 0f && worldTime < (float)waitFirstBackup)
		{
			ZLog.Log((object)"Skipping backup. World session not long enough.");
			return false;
		}
		if (backupCount == 1)
		{
		...
		{
			ZLog.LogError((object)("Failed to retrieve save with name " + saveName + "!"));
			return false;
		}
		if (save.IsDeleted)
		{
			ZLog.LogError((object)("Save with name " + saveName + " is deleted, can't manage auto-backups!"));
			return false;
		}
		...
		if (list.Count == 0)
		{
			ZLog.Log((object)"Creating first autobackup");
		}
		else
		{
			if (!(now - TimeSpan.FromSeconds(backupShort) > list[0].LastModified))
			{
				ZLog.Log((object)"No autobackup needed yet...");
				return false;
			}
			if (list.Count == 1)
			{
				ZLog.Log((object)"Creating second autobackup for reference");
			}
			else if (now - TimeSpan.FromSeconds(backupLong) > list[1].LastModified)
			{
				if (list.Count < backupCount)
				{
					ZLog.Log((object)"Creating new backup since we haven't reached our desired amount");
				}
				else
				{
					saveFile2 = list[list.Count - 1];
					ZLog.Log((object)"Time to overwrite our last autobackup");
				}
			}
			else
			{
				saveFile2 = list[0];
				ZLog.Log((object)"Overwrite our newest autobackup since the second one isn't so old");
			}
		}
		if (saveFile2 != null)
		{
			ZLog.Log((object)("Replacing backup file: " + saveFile2.FileName));
			if (!Delete(saveFile2))
			{
				ZLog.LogError((object)("Failed to delete backup " + saveFile2.FileName + "!"));
				return false;
			}
		}
		string text = saveName + "_backup_auto-" + now.ToString("yyyyMMddHHmmss");
		ZLog.Log((object)("Saving backup at: " + text));
		if (!Copy(save.PrimaryFile, text, save.PrimaryFile.m_source))
		{
			ZLog.LogError((object)("Failed to copy save with name " + saveName + " to auto-backup!"));
			return false;
		}
		return true;
	}
	...
	private static bool IsRestorableBackup(SaveFile backup)
	{
		...
		case SaveDataType.Character:
			...
			break;
		default:
			ZLog.LogError((object)$"Not implemented for {backup.ParentSaveWithBackups.ParentSaveCollection.m_dataType}!");
			return false;
		}
	...
	public static bool GetSaveInfo(string path, out string saveName, out SaveFileType saveFileType, out string actualFileEnding, out DateTime? timestamp)
	{
		...
		int[] array = StringExtensionMethods.AllIndicesOf(text, '.');
		if (array.Length == 0)
		{
			saveName = "";
			actualFileEnding = "";
		...
		int[] array3 = StringExtensionMethods.AllIndicesOf(text, '_');
		if (array3.Length >= 1)
		{
	...
	public static bool TryConvertSource(string sourcePath, FileSource sourceLocation, FileSource destinationLocation, out string destinationPath)
	{
	...
	public static string NormalizePath(string path, FileSource source)
	{
	...
	public static List<PlayerProfile> GetAllPlayerProfiles()
	{
		...
		for (int i = 0; i < savesByType.Length; i++)
		{
			if (!savesByType[i].IsDeleted)
			{
				...
				{
					ZLog.Log((object)("Failed to load " + savesByType[i].m_name));
				}
				else
				{
	...
	public static bool CanSaveToCloudStorage(World world, PlayerProfile playerProfile)
	{
		...
		bool flag = world != null && (!FileHelpers.LocalStorageSupported || (int)world.m_fileSource == 2 || (FileHelpers.CloudStorageEnabled && (int)world.m_fileSource == 3));
		bool flag2 = playerProfile != null && (!FileHelpers.LocalStorageSupported || (int)playerProfile.m_fileSource == 2 || (FileHelpers.CloudStorageEnabled && (int)playerProfile.m_fileSource == 3));
		if (!flag && !flag2)
		{
			return true;
		}
		...
		if (flag)
		{
			...
			{
				ZLog.LogError((object)"World save file doesn't exist! Using less accurate storage usage estimate.");
			}
		}
		if (flag2)
		{
			...
			{
				ZLog.LogError((object)"Player save file doesn't exist! Using less accurate storage usage estimate.");
			}
		}
