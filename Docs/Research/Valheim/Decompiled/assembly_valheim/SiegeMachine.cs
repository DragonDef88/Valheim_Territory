...
public class SiegeMachine : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		m_aoe.gameObject.SetActive(false);
	...
	private void UpdateSiege(float dt)
	{
		...
		bool flag = m_nview != null && m_nview.IsValid() && m_nview.IsOwner();
		foreach (SiegePart movingPart in m_movingParts)
		{
			movingPart.m_dynamicsPosition = movingPart.m_floatDynamics.Update(dt, m_armAnimCurve.Evaluate(movingPart.m_position / m_chargeTime), float.NegativeInfinity, false);
		...
		if ((Object.op_Implicit((Object)(object)m_engine) && !m_engine.IsActive()) || (Object.op_Implicit((Object)(object)m_wagon) && (m_enabledWhenAttached ^ m_wagon.IsAttached())))
		{
			m_aoe.gameObject.SetActive(false);
		...
		else
		{
			...
			if (m_firingTimer > m_hitDelay)
			{
				...
				if (flag)
				{
					...
					m_aoe.gameObject.SetActive(true);
				}
				UpdateAnimPhase();
			}
		}
