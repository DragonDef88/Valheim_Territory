...
public class MasterClient
{
	...
	private void SendStats(float duration)
	{
		...
		zPackage.Write(36u);
		bool flag3 = Object.op_Implicit((Object)(object)ZNet.instance) && ZNet.instance.IsServer();
		...
		PlayerProfile playerProfile = (((Object)(object)Game.instance != (Object)null) ? Game.instance.GetPlayerProfile() : null);
	...
	public void RegisterServer(string name, string host, int port, bool password, bool upnp, long worldUID, GameVersion gameVersion, uint networkVersion, string[] modifiers)
	{
		...
		m_registerPkg.Write(StringUtils.EncodeStringListAsString((IReadOnlyList<string>)modifiers, true));
		if (m_rpc != null)
		{
	...
	private void RPC_ServerList(ZRpc rpc, ZPackage pkg)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				StringUtils.TryDecodeStringAsArray(pkg.ReadString(), ref modifiers);
			}
			else
			{
			...
			ServerMatchmakingData matchmakingData = new ServerMatchmakingData(DateTime.UtcNow, text, (uint)playerCount, 10u, PlatformUserID.None, version, networkVersion, null, isPasswordProtected, Platform.Unknown, modifiers);
