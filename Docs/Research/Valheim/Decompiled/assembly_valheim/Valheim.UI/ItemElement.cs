...
public class ItemElement : RadialMenuElement
{
	...
	protected virtual void SetAmount(ItemDrop.ItemData item)
	{
		...
		{
			((Component)m_amount).gameObject.SetActive(true);
		...
		{
			((Component)m_amount).gameObject.SetActive(false);
		}
	}
	...
	protected virtual void SetDurability(ItemDrop.ItemData item)
	{
		...
		((Component)m_durability).gameObject.SetActive(flag);
		if (flag)
		{
			if (item.m_durability <= 0f)
			{
				m_durability.SetValue(1f);
				m_durability.SetColor((Color)((Mathf.Sin(Time.time * 10f) > 0f) ? Color.red : new Color(0f, 0f, 0f, 0f)));
			}
			else
			{
