...
public class ZRoutedRpc
{
	...
	{
		m_functions.Add(StringExtensionMethods.GetStableHashCode(name), new RoutedMethod<T, U, V, B, K>(f));
	}

	public void Register<T, U, V, B, K, M>(string name, RoutedMethod<T, U, V, B, K, M>.Method f)
	{
		m_functions.Add(StringExtensionMethods.GetStableHashCode(name), new RoutedMethod<T, U, V, B, K, M>(f));
	}
}
