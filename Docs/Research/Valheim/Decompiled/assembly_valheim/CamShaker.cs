...
public class CamShaker : MonoBehaviour
{
	...
	private void Start()
	{
		if (m_continous)
		{
			if (m_delay <= 0f)
			{
				((MonoBehaviour)this).StartCoroutine("TriggerContinous");
			}
			else
			{
				((MonoBehaviour)this).Invoke("DelayedTriggerContinous", m_delay);
			}
		}
		else if (m_delay <= 0f)
		{
		...
		{
			((MonoBehaviour)this).Invoke("Trigger", m_delay);
		}
	}
	...
	private void Trigger()
	{
		...
		GameCamera.instance.AddShake(((Component)this).transform.position, m_range, m_strength, m_continous);
	}
}
