...
public class WispSpawner : MonoBehaviour, Hoverable
{
	...
	private void Start()
	{
		...
		((MonoBehaviour)this).InvokeRepeating("TrySpawn", 10f, 10f);
		((MonoBehaviour)this).InvokeRepeating("UpdateDemister", Random.Range(0f, 2f), 2f);
	}
	...
	private void UpdateDemister()
	{
		...
		if (Object.op_Implicit((Object)(object)m_wispsNearbyObject))
		{
	...
	private void TrySpawn()
	{
		...
		if (m_nview.IsValid() && m_nview.IsOwner())
		{
			...
			if (!((time - dateTime).TotalSeconds < (double)m_spawnInterval) && !(Random.value > m_spawnChance) && GetStatus() == Status.Ok)
			{
				Vector3 val = m_spawnPoint.position + Quaternion.Euler(0f, (float)Random.Range(0, 360), 0f) * Vector3.forward * m_spawnDistance;
				Object.Instantiate<GameObject>(m_wispPrefab, val, Quaternion.identity);
	...
	public static WispSpawner GetBestSpawner(Vector3 p, float maxRange)
	{
		...
		foreach (WispSpawner s_spawner in s_spawners)
		{
			...
			if (!(num2 > maxRange))
			{
				...
				if (status != 0 && status != Status.TooBright && (status != Status.Full || !(num2 > s_spawner.m_maxSpawnedArea)) && (num2 < num || (Object)(object)wispSpawner == (Object)null))
				{
					num = num2;
