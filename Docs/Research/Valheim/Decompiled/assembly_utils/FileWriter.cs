...
public class FileWriter
{
	...
	public FileWriter(string path, FileHelpers.FileHelperType type = FileHelpers.FileHelperType.Binary, FileHelpers.FileSource fileSource = FileHelpers.FileSource.Auto)
	{
		m_path = path;
		m_type = type;
		m_fileSource = ((!FileHelpers.CloudStorageEnabled || (fileSource != 0 && fileSource != FileHelpers.FileSource.Cloud)) ? FileHelpers.FileSource.Local : FileHelpers.FileSource.Cloud);
		if (fileSource == FileHelpers.FileSource.Legacy)
		{
			Status = WriterStatus.OpenFailed;
			return;
		}
		if (FileHelpers.CloudStorageSupported && m_fileSource == FileHelpers.FileSource.Cloud)
		{
